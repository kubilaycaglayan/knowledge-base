...
missing_env=0
for env_name in DOMAIN JWT_SECRET POSTGRES_PASSWORD CLOUDFLARE_TUNNEL_TOKEN KNOW_API_BASE BACKUP_DB_URL BACKUP_DB_CONFIRM; do
  if [[ -z "${!env_name:-}" ]]; then
...
for backup_db_url_name in BACKUP_DB_URL; do
  backup_db_url="${!backup_db_url_name}"
...
case "$BACKUP_DB_URL" in
  ...
  *) fail 'BACKUP_DB_URL must include a database name in its path' ;;
esac
