...
final class KnowUITests: XCTestCase {
  ...
  func testPasswordVisibilityCanBeToggled() {
    ...
    XCTAssertEqual(
      (app.secureTextFields["auth.password"].value as? String)?.count, "password123".count)
    XCTAssertEqual(visibility.label, "Show password")
  }
  ...
  func testReportsDestinationAndAccessibleSummaryAreReachable() {
    ...
    let bucket = app.descendants(matching: .any).matching(
      NSPredicate(format: "identifier BEGINSWITH 'reports.bucket.'")
    ).firstMatch
    XCTAssertTrue(bucket.waitForExistence(timeout: 5))
    XCTAssertTrue(bucket.label.contains("Research"))
    let breakdown = app.buttons["reports.breakdown"]
    XCTAssertTrue(breakdown.waitForExistence(timeout: 5))
    XCTAssertGreaterThanOrEqual(breakdown.frame.height, 44)
    breakdown.tap()
    let labelChoices = app.buttons.matching(NSPredicate(format: "label == 'Labels'"))
    XCTAssertTrue(labelChoices.count > 1)
    labelChoices.element(boundBy: labelChoices.count - 1).tap()
  ...
  func testReportsLiveAPIFlowWhenExplicitlyConfigured() throws {
    ...
    guard let baseURL = environment["KNOW_PHYSICAL_REPORTS_API_URL"],
      ...
      let password = environment["KNOW_PHYSICAL_REPORTS_PASSWORD"]
    else {
    ...
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label BEGINSWITH 'Path timing by'")).firstMatch
  ...
  func testReportsRemainUsableWithReducedMotion() {
    app.launchArguments += [
      "-ui-testing-authenticated", "-UIAccessibilityReduceMotionEnabled", "YES",
    ]
    app.launch()
    app.buttons["workspace.reports"].tap()
    XCTAssertTrue(app.descendants(matching: .any)["reports.page"].waitForExistence(timeout: 5))
  ...
  func testReportsUseLargePhoneWidthWithoutHorizontalOverflow() {
    app.launchArguments += ["-ui-testing-authenticated"]
    app.launch()
    app.buttons["workspace.reports"].tap()
    let page = app.descendants(matching: .any)["reports.page"]
  ...
  func testReportsEmptyStateAndRecoverableErrorFixtures() {
    ...
    app.terminate()
    app = XCUIApplication()
    app.launchArguments = ["-ui-testing", "-ui-testing-authenticated", "-reports-error"]
    app.launch()
    app.buttons["workspace.reports"].tap()
    XCTAssertTrue(
      app.staticTexts["Unable to load the report. Please try again."].waitForExistence(timeout: 5))
  ...
  func testReportsCalendarAndSankeyFixturesExposeTextualDetails() {
    ...
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'marked day'")).firstMatch.exists
    )
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Planning'")).firstMatch.exists)
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Deep focus'")).firstMatch.exists
    )
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Marked'")).firstMatch.exists)
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Calendar input'")).firstMatch
        .exists)
    app.buttons["reports.calendar.toggle"].tap()
    ...
    XCTAssertTrue(
      app.staticTexts.matching(
        NSPredicate(format: "label CONTAINS 'Flow from Research to Deep work'")
      ).firstMatch.exists)
  ...
  func testReportsCalendarNoteOnlyAndFractionFixturesExposeDistinctDetails() {
    ...
    app.buttons["workspace.reports"].tap()
    XCTAssertTrue(app.buttons["reports.calendar.toggle"].waitForExistence(timeout: 5))
    for portion in ["0.25 days", "0.5 days", "0.75 days", "1 day"] {
      XCTAssertTrue(
        app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", portion)).firstMatch
          .exists)
    }
    app.terminate()
    app = XCUIApplication()
    app.launchArguments = [
      "-ui-testing", "-ui-testing-authenticated", "-reports-calendar-note-only",
    ]
    app.launch()
    app.buttons["workspace.reports"].tap()
    XCTAssertTrue(app.buttons["reports.calendar.toggle"].waitForExistence(timeout: 5))
    XCTAssertTrue(app.staticTexts["Planning day\nReview the weekly priorities."].exists)
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Calendar input'")).firstMatch
        .exists)
  }
  ...
  func testReportsReferenceFailureFallsBackToReportCategories() {
    app.launchArguments += [
      "-ui-testing-authenticated", "-reports-paths-error", "-reports-labels-error",
    ]
    app.launch()
    app.buttons["workspace.reports"].tap()
  ...
  func testReportsLongFilterOptionsKeepAccessibleRemovalControls() {
    ...
    app.launch()
    app.buttons["workspace.reports"].tap()
    app.buttons["reports.paths.filter"].tap()
    let pathOption = app.buttons.matching(
      NSPredicate(format: "label CONTAINS 'Research and planning with a very long path name'")
    ).firstMatch
    XCTAssertTrue(pathOption.waitForExistence(timeout: 5))
    pathOption.tap()
    XCTAssertTrue(
      app.buttons["reports.paths.remove.00000000-0000-0000-0000-000000000001"].waitForExistence(
        timeout: 5))
    ...
    app.buttons["reports.labels.filter"].tap()
    let labelOption = app.buttons.matching(
      NSPredicate(format: "label CONTAINS 'Deep work label with a long accessible name'")
    ).firstMatch
    XCTAssertTrue(labelOption.waitForExistence(timeout: 5))
    labelOption.tap()
    XCTAssertTrue(
      app.buttons["reports.labels.remove.00000000-0000-0000-0000-000000000011"].waitForExistence(
        timeout: 5))
    XCTAssertTrue(
      app.buttons["reports.labels.remove.00000000-0000-0000-0000-000000000011"].isHittable)
    app.buttons["reports.breakdown"].tap()
    let labelChoices = app.buttons.matching(NSPredicate(format: "label == 'Labels'"))
    XCTAssertTrue(labelChoices.count > 1)
    labelChoices.element(boundBy: labelChoices.count - 1).tap()
    XCTAssertTrue(
  ...
  func testReportsZeroAndLongRangeFixturesRemainNavigable() {
    ...
    app.terminate()
    app = XCUIApplication()
    app.launchArguments = ["-ui-testing", "-ui-testing-authenticated", "-reports-long-range"]
    app.launch()
    app.buttons["workspace.reports"].tap()
  ...
  func testReportsDenseFixtureKeepsLargeCategoryCollectionsReadable() {
    ...
    app.buttons["reports.breakdown"].tap()
    let choices = app.buttons.matching(NSPredicate(format: "label == 'Labels'"))
    XCTAssertTrue(choices.count > 1)
  ...
  func testReportsGeneratedContentRemainsLiteralText() {
    ...
    app.buttons["workspace.reports"].tap()
    app.buttons["reports.paths.filter"].tap()
    let unsafePath = app.buttons.matching(
      NSPredicate(format: "label CONTAINS '<script>alert(1)</script>'")
    ).firstMatch
    XCTAssertTrue(unsafePath.waitForExistence(timeout: 5))
    XCTAssertTrue(unsafePath.label.contains("<script>alert(1)</script>"))
    unsafePath.tap()
    app.swipeUp()
    app.buttons["reports.breakdown"].tap()
    let labelChoices = app.buttons.matching(NSPredicate(format: "label == 'Labels'"))
    ...
    labelChoices.element(boundBy: labelChoices.count - 1).tap()
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS '<b>Deep work</b>'")).firstMatch
        .waitForExistence(timeout: 5))
    XCTAssertTrue(app.staticTexts["Calendar log"].waitForExistence(timeout: 5))
    XCTAssertTrue(app.staticTexts["<em>Planning</em>"].exists)
  }
  ...
  func testReportsLoadingSkeletonAndEmptySankeyRemainRecoverable() {
    ...
    XCTAssertTrue(
      app.descendants(matching: .any)["reports.status"].label.contains("Loading report"))

    app.terminate()
    app = XCUIApplication()
    app.launchArguments = [
      "-ui-testing", "-ui-testing-authenticated", "-reports-empty", "-reports-sankey",
    ]
    app.launch()
    app.buttons["workspace.reports"].tap()
    XCTAssertTrue(app.staticTexts["No report data for this period."].waitForExistence(timeout: 5))
  ...
  func testReportsFiltersAndPresentationRestoreAcrossWorkspaceSections() {
    ...
    XCTAssertTrue(pathsFilter.waitForExistence(timeout: 5))
    pathsFilter.tap()
    XCTAssertTrue(app.buttons["Research"].waitForExistence(timeout: 3))
    app.buttons["Research"].tap()
    pathsFilter.tap()
    XCTAssertTrue(app.buttons["Writing"].waitForExistence(timeout: 3))
    ...
    XCTAssertTrue(app.buttons["reports.trendline"].label.contains("Linear"))
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'Linear trend'")).firstMatch
        .exists)
    let labelsFilter = app.buttons["reports.labels.filter"]
    labelsFilter.tap()
    XCTAssertTrue(app.buttons["Deep work"].waitForExistence(timeout: 3))
    app.buttons["Deep work"].tap()
    labelsFilter.tap()
    XCTAssertTrue(app.buttons["Planning"].waitForExistence(timeout: 3))
    ...
    XCTAssertTrue(app.buttons["reports.labels.remove.00000000-0000-0000-0000-000000000012"].exists)
    app.buttons["workspace.paths"].tap()
    XCTAssertTrue(app.buttons["workspace.paths"].isSelected)
    app.buttons["workspace.reports"].tap()
    XCTAssertTrue(remove.waitForExistence(timeout: 5))
    XCTAssertTrue(app.buttons["reports.trendline"].label.contains("Linear"))
  ...
  func testCalendarSavedFixtureShowsAccessibleDayAndRangeControls() {
    ...
    XCTAssertTrue(dayLabel.contains("2026"))
    XCTAssertTrue(dayLabel.contains("note"))
    XCTAssertTrue(dayLabel.contains("1 label"))
  ...
  func testCalendarLoadingAndExpiredSessionFixtures() {
    ...
    app.terminate()
    app = XCUIApplication()
    app.launchArguments = ["-ui-testing", "-ui-testing-authenticated", "-calendar-unauthorized"]
    app.launch()
    XCTAssertTrue(app.textFields["auth.email"].waitForExistence(timeout: 5))
  ...
  func testCalendarPortionsRangeAlternativeAndFailedSaveRetainDraft() {
    ...
    note.tap()
    note.typeText("Retained draft")
    app.buttons["Select Milestone"].tap()
    let portion = app.buttons["calendar.portion.00000000-0000-4000-8000-000000000020"]
    XCTAssertTrue(portion.waitForExistence(timeout: 3))
    portion.tap()
    for option in ["Marker only", "¼ day", "½ day", "¾ day", "Full day"] {
    ...
    app.buttons["Select range"].tap()
    XCTAssertTrue(app.staticTexts["Release on another day to select a range."].exists)
  ...
  func testCalendarCreatesLabelWithCustomPaletteColor() {
    ...
    app.buttons.matching(NSPredicate(format: "label BEGINSWITH 'Choose new label color'"))
      .firstMatch.tap()
    XCTAssertTrue(app.buttons["Choose new label color: #F97316"].waitForExistence(timeout: 3))
    app.buttons["Choose new label color: #F97316"].tap()
    let field = app.textFields["New calendar label"]
    field.tap()
  ...
  func testCalendarControlsRemainReachableInDarkAccessibilityAppearance() {
    app.launchArguments += [
      "-ui-testing", "-ui-testing-authenticated", "-AppleInterfaceStyle", "Dark",
  ...
  func testCalendarLightSystemAndReducedMotionAppearances() {
    for arguments in [
      ["-ui-testing", "-ui-testing-authenticated", "-AppleInterfaceStyle", "Light"],
      ["-ui-testing", "-ui-testing-authenticated"],
      ["-ui-testing", "-ui-testing-authenticated", "-UIAccessibilityReduceMotionEnabled", "YES"],
    ] {
  ...
  func testPathsEmptyAndOfflineFixturesOfferRecovery() {
    ...
    app.buttons["workspace.paths"].tap()
    XCTAssertTrue(
      app.staticTexts["Your first path is waiting to be named."].waitForExistence(timeout: 5))
    ...
    app.buttons["workspace.paths"].tap()
    XCTAssertTrue(
      app.staticTexts["No network connection. Reconnect and try again."].waitForExistence(
        timeout: 5))
  }
  ...
  func testPathsControlsRemainReachableAtAccessibilityTextSize() {
    app.launchArguments += [
      "-ui-testing-authenticated", "-UIPreferredContentSizeCategory",
      "UICTContentSizeCategoryAccessibilityXXXL",
    ]
    app.launch()
    app.buttons["workspace.paths"].tap()
  ...
  func testLogsComposerEditDeleteAndLabelsAreReachable() {
    ...
    XCTAssertTrue(
      app.staticTexts["No logs yet. Capture a thought above."].waitForExistence(timeout: 3))
  }
  ...
  func testLogsEmptyAndOfflineFixturesOfferRecovery() {
    ...
    app.buttons["workspace.logs"].tap()
    XCTAssertTrue(
      app.staticTexts["No logs yet. Capture a thought above."].waitForExistence(timeout: 5))
    app.terminate()
    app = XCUIApplication()
  ...
  func testLabelsEmptyAndOfflineFixturesOfferRecovery() {
    ...
    XCTAssertTrue(
      app.staticTexts["No labels yet. Add one above to get started."].waitForExistence(timeout: 5))
    app.terminate()
  ...
  func testSessionEditPersistsAndDeletionRequiresConfirmation() {
    ...
    edit.tap()
    let description = app.descendants(matching: .any).matching(identifier: "session.description")
      .firstMatch
    ...
    XCTAssertTrue(
      app.buttons["session.edit.00000000-0000-4000-8000-000000000003"].waitForExistence(timeout: 5))
