...
@MainActor final class CalendarTests: XCTestCase {
  final class Stub: CalendarTransport {
    ...
    func days(startDate: String, endDate: String) async throws -> [CalendarDay] {
      if let failure { throw failure }
      loads += 1
      loadedDayRanges.append((startDate, endDate))
      return daysValue
    }
    func saveDay(date: String, request: CalendarDayRequest) async throws -> CalendarDay {
      if let failure { throw failure }
      dayRequests.append((date, request))
      return CalendarDay(
        date: date, note: request.note,
        labels: request.labels.map {
          CalendarAssignment(
            labelId: $0.labelId, name: "Work", color: "#2878D5", portion: $0.portion)
        })
    }
    func saveRange(request: CalendarRangeRequest) async throws -> [CalendarDay] {
      if let failure { throw failure }
      rangeRequests.append(request)
      return []
    }
    func createLabel(name: String, color: String) async throws -> KBLabel {
      KBLabel(id: UUID(), name: name, color: color, scopes: [.calendar])
    }
  ...
  final class DelayedStub: CalendarTransport, @unchecked Sendable {
    ...
    private var dayCall = 0
    private var saveCall = 0
    ...
    private func nextDayCall() -> Int {
      lock.lock()
      defer { lock.unlock() }
      dayCall += 1
      return dayCall
    }
    private func incrementSaveCall() {
      lock.lock()
      defer { lock.unlock() }
      saveCall += 1
    }
  ...
  func testMondayFirstGridHasSixWeeksAndAdjacentDays() {
    let cal = calendar()
    let date = cal.date(from: DateComponents(year: 2026, month: 9, day: 13))!
  ...
  func testLocalDateRoundTripDoesNotUseUTCInstant() {
    ...
    let date = cal.date(from: DateComponents(year: 2026, month: 3, day: 8))!
    XCTAssertEqual(CalendarDate.string(date, calendar: cal), "2026-03-08")
    XCTAssertEqual(
      CalendarDate.string(cal.date(byAdding: .day, value: 1, to: date)!, calendar: cal),
      "2026-03-09")
  }
  ...
  func testCodablePreservesNullableNoteColorAndPortions() throws {
    let id = UUID()
    let day = CalendarDay(
      date: "2026-09-03", note: nil,
      labels: [CalendarAssignment(labelId: id, name: "Work", color: nil, portion: 0.75)])
    let decoded = try JSONDecoder().decode(CalendarDay.self, from: JSONEncoder().encode(day))
    XCTAssertEqual(decoded, day)
    XCTAssertEqual(decoded.labels[0].portionTitle, "¾ day")
    let request = CalendarDayRequest(
      note: nil, labels: [CalendarDayAssignment(labelId: id, portion: nil)])
  ...
  func testLoadHydratesDraftAndReusesCachedRange() async {
    let stub = Stub()
    let id = stub.labelsValue[0].id
    stub.daysValue = [
      CalendarDay(
        date: "2026-09-03", note: "Saved",
        labels: [CalendarAssignment(labelId: id, name: "Work", color: nil, portion: 0.25)])
    ]
    let cal = calendar()
    let now = cal.date(from: DateComponents(year: 2026, month: 9, day: 3))!
    let model = CalendarModel(transport: stub, now: now, calendar: cal)
    await model.load()
    XCTAssertEqual(stub.loadedDayRanges.first?.0, "2026-09-01")
    XCTAssertEqual(stub.loadedDayRanges.first?.1, "2026-09-30")
    XCTAssertEqual(model.note, "Saved")
    XCTAssertEqual(model.selectedAssignments[id], .quarter)
    XCTAssertFalse(model.hasUnsavedDraft)
    let count = stub.loads
    await model.load()
    XCTAssertEqual(stub.loads, count)
  }
  ...
  func testRefreshPreservesDirtyDraftAndEmptyResponseClearsLoadedMonth() async {
    ...
    XCTAssertEqual(model.note, "Unsaved")
    XCTAssertNil(model.days["2026-09-03"])
  ...
  func testBlankNoteTrimsToNullAndPortionToggleSaves() async {
    let stub = Stub()
    let model = CalendarModel(transport: stub, now: Date(), calendar: calendar())
    model.note = "  \n "
    let label = stub.labelsValue[0]
    model.toggle(label)
    model.setPortion(.half, for: label)
    let saved = await model.save()
    XCTAssertTrue(saved)
    XCTAssertEqual(stub.dayRequests[0].1.note, nil)
    XCTAssertEqual(stub.dayRequests[0].1.labels[0].portion, 0.50)
  }
  ...
  func testSaveCapsLongNoteAndCreateLabelRejectsInvalidColorAndDuplicates() async {
    ...
    XCTAssertEqual(
      model.labels.filter { $0.name.caseInsensitiveCompare("Launch") == .orderedSame }.count, 1)
  }

  func testRangeNormalizesSameDayFallsBackAndRetainsDraftOnFailure() async {
    let stub = Stub()
    let cal = calendar()
    let a = cal.date(from: DateComponents(year: 2026, month: 9, day: 3))!
    let b = cal.date(from: DateComponents(year: 2026, month: 9, day: 1))!
    let model = CalendarModel(transport: stub, now: a, calendar: cal)
    model.beginRange(a)
    XCTAssertTrue(model.selectingRange)
    let incomplete = await model.save()
    XCTAssertFalse(incomplete)
    XCTAssertTrue(stub.dayRequests.isEmpty)
    model.select(b)
    XCTAssertEqual(CalendarDate.string(model.selectedDate, calendar: cal), "2026-09-01")
    model.note = "Range note"
    let firstSave = await model.save()
    XCTAssertTrue(firstSave)
    XCTAssertEqual(stub.rangeRequests[0].startDate, "2026-09-01")
    XCTAssertEqual(stub.rangeRequests[0].endDate, "2026-09-03")
    XCTAssertEqual(CalendarDate.string(model.selectedDate, calendar: cal), "2026-09-01")
    model.beginRange(a)
    model.select(a)
    XCTAssertFalse(model.isRangeMode)
    stub.failure = APIError.offline
    model.beginRange(a)
    model.select(b)
    model.note = "Keep me"
    ...
    XCTAssertEqual(model.note, "Keep me")
  ...
  func testLabelCreateTrimsAndColorUpdateUsesFixedPalette() async {
    let stub = Stub()
    let model = CalendarModel(transport: stub, calendar: calendar())
    let created = await model.createLabel(name: "  Launch  ", color: WorkspaceTheme.palette[2])
    XCTAssertTrue(created)
    XCTAssertEqual(model.labels.last?.name, "Launch")
    XCTAssertEqual(model.labels.last?.color, WorkspaceTheme.palette[2])
    let label = model.labels[0]
    let recolored = await model.updateLabelColor(label, color: WorkspaceTheme.palette[4])
    XCTAssertTrue(recolored)
    XCTAssertEqual(model.labels[0].color, WorkspaceTheme.palette[4])
    let rejected = await model.updateLabelColor(label, color: "#not-a-palette-color")
  ...
  func testUnauthorizedLoadCallsSignOutAndMutationInvalidatesReports() async {
    ...
    let model = CalendarModel(
      transport: stub, calendar: calendar(), unauthorized: { signedOut = true },
      invalidateReports: { invalidated += 1 })
    stub.failure = APIError.unauthorized
    await model.load()
    XCTAssertTrue(signedOut)
    stub.failure = nil
    let created = await model.createLabel(name: "New")
    XCTAssertTrue(created)
  ...
  func testMonthChangeSelectsFirstDayAndCachedReturnHydratesIt() async {
    let stub = Stub()
    let cal = calendar()
    let september = cal.date(from: DateComponents(year: 2026, month: 9, day: 13))!
    let augustFirst = cal.date(from: DateComponents(year: 2026, month: 8, day: 1))!
    stub.daysValue = [CalendarDay(date: "2026-08-01", note: "Month start", labels: [])]
    let model = CalendarModel(transport: stub, now: september, calendar: cal)
    await model.setMonth(augustFirst)
    XCTAssertEqual(model.note, "Month start")
    await model.setMonth(september)
    await model.setMonth(augustFirst)
    XCTAssertEqual(CalendarDate.string(model.selectedDate, calendar: cal), "2026-08-01")
    XCTAssertEqual(model.note, "Month start")
  }
  ...
  func testNewerMonthSuppressesStaleLoadAndConcurrentSaveIsRejected() async {
    ...
    await model.setMonth(september)
    await oldLoad.value
    XCTAssertNil(model.days["2026-08-01"])
    XCTAssertEqual(model.days["2026-09-01"]?.note, "Current")
    XCTAssertEqual(model.note, "Current")
    model.note = "One"
