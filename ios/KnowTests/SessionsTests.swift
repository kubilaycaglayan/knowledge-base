...
private actor SessionsStub: SessionsTransport {
    ...
    func history(page: Int) async throws -> SessionPage {
        pages.append(page)
        return SessionPage(sessions: page == 0 ? [sample()] : [], page: page, totalPages: 1, totalSessions: 1)
    }
    ...
    func releaseCurrent(_ value: TrackedSession?) { heldCurrent?.resume(returning: value); heldCurrent = nil }
    ...
    func createLabel(name: String) async throws -> SessionLabel { lastLabelName = name; return label }
    func sample() -> TrackedSession {
        TrackedSession(id: UUID(uuidString: "00000000-0000-4000-8000-000000000009")!, pathId: path.id, labelIds: [label.id], startedAt: "2026-09-12T10:00:00Z", endedAt: "2026-09-12T11:00:00Z", durationSeconds: 3600, description: "Original", source: "WEB", running: false)
    }
}
...
@MainActor final class SessionsTests: XCTestCase {
    func testLoadUsesScopedLabelsAndOnlyActiveRecentPaths() async {
        ...
        await model.load()
        let path = await stub.path, archived = await stub.archived
    ...
    func testSocketReadyInvalidatesInflightPollAndCompletedSnapshotsClearTimer() async throws {
        ...
        let data = try JSONSerialization.data(withJSONObject: ["type": "TIMER_STATE", "timer": JSONSerialization.jsonObject(with: JSONEncoder().encode(current))])
    ...
    func testCreationTrimsAndAssignsWithoutDuplicateLabels() async {
        ...
        _ = await model.createLabel("Focus")
        let pathName = await stub.lastPathName, labelName = await stub.lastLabelName
    ...
    func testDateGroupsUseLocalDayAndMondayWeek() {
        ...
        XCTAssertEqual(SessionFormatting.group("2026-09-13T22:00:00Z", now: now, calendar: calendar), "Today")
        XCTAssertEqual(SessionFormatting.group("2026-09-13T10:00:00Z", now: now, calendar: calendar), "Yesterday")
        XCTAssertEqual(SessionFormatting.group("2026-09-08T10:00:00Z", now: now, calendar: calendar), "Last week")
        XCTAssertEqual(SessionFormatting.group("2026-08-20T10:00:00Z", now: now, calendar: calendar), "Last month")
    }
}
