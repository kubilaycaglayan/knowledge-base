...
private actor PathsStub: PathsTransport {
    ...
    func paths() async throws -> [Path] { try checkFailure(); if fail { throw APIError.offline }; return values }
    func summary(id: UUID) async throws -> PathSummary { try checkFailure(); return PathSummary(path: values[0], trackedSeconds: 120, recentActivity: []) }
    func create(name: String, description: String?, color: String) async throws -> Path { try checkFailure(); let value = Path(id: UUID(), name: name, description: description, status: "ACTIVE", color: color, activityLabel: "passive"); values.insert(value, at: 0); return value }
    func update(id: UUID, name: String, description: String?, color: String) async throws -> Path { try checkFailure(); let value = Path(id: id, name: name, description: description, status: "ACTIVE", color: color); values = values.map { $0.id == id ? value : $0 }; return value }
    func remove(id: UUID) async throws { try checkFailure(); removed = id; values.removeAll { $0.id == id } }
    func restore(id: UUID) async throws { try checkFailure(); restored = id }
    func merge(source: UUID, target: UUID) async throws { try checkFailure(); merged = (source, target) }
...
@MainActor final class PathsTests: XCTestCase {
    ...
    func testUnauthorizedExpiresSessionAndServerFailurePreservesRetryState() async {
        ...
        var invalidations = 0
        let model = PathsModel(transport: stub, unauthorized: { unauthorizedCalls += 1 }, invalidateReports: { invalidations += 1 })
        ...
        let updated = await model.update(model.paths[0], name: "Retry", description: "", color: "#3B82F6")
...
private extension PathsStub {
    ...
    func setFailure(_ value: APIError) { failure = value }
}
