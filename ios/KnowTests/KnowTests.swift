...
final class KnowTests: XCTestCase {
  func testAPIClientPreservesQueryItemsAndDecodesNullTimer() async throws {
    ...
    URLProtocolStub.responseData = Data(
      "{\"sessions\":[],\"page\":1,\"totalPages\":2,\"totalSessions\":51}".utf8)
  ...
  func testSessionDraftEncodesExplicitClearsAndMultipleLabels() throws {
    ...
    let fields = try XCTUnwrap(
      JSONSerialization.jsonObject(with: draft.body(completed: true)) as? [String: Any])
    ...
    let running = try XCTUnwrap(
      JSONSerialization.jsonObject(with: draft.body(completed: false)) as? [String: Any])
  ...
  func testGoogleConfigurationRequiresMatchingIDsAndCallbackScheme() {
    ...
    XCTAssertTrue(
      GoogleAuthentication.configurationIsValid(
        clientID: client, serverID: server, urlSchemes: ["com.googleusercontent.apps.123"]))
    XCTAssertFalse(
      GoogleAuthentication.configurationIsValid(
        clientID: client, serverID: nil, urlSchemes: ["com.googleusercontent.apps.123"]))
    XCTAssertFalse(
      GoogleAuthentication.configurationIsValid(clientID: client, serverID: server, urlSchemes: []))
  }
  ...
  @MainActor private func authenticationModel() -> AppModel {
    ...
    return AppModel(
      api: APIClient(
        base: URL(string: "https://example.test/api/v1")!,
        session: URLSession(configuration: configuration)), arguments: ["-ui-testing"])
  }

  private var authPayload: Data {
    Data(
      "{\"token\":\"backend-session\",\"userId\":\"00000000-0000-4000-8000-000000000001\",\"email\":\"person@example.com\",\"displayName\":\"Person\"}"
        .utf8)
  }
  ...
  func testIOSTimerRequestEncodesCanonicalSourceAndTargets() throws {
    ...
    let request = TimerRequest(
      pathId: nil, labelIds: [labelId.uuidString], description: "Reading", source: "IOS")
  ...
  func testLogsAPIUsesAuthenticatedContractAndFullLabelReplacement() async throws {
    ...
    let response =
      "{\"id\":\"\(id.uuidString)\",\"body\":\"Read\",\"occurredAt\":\"2026-09-13T10:00:00Z\",\"labelIds\":[],\"createdAt\":\"2026-09-13T10:00:00Z\",\"updatedAt\":\"2026-09-13T10:00:00Z\",\"version\":3}"
    ...
    let api = LogsAPI(
      client: APIClient(
        base: URL(string: "https://example.test/api/v1")!,
        session: URLSession(configuration: configuration)), token: "test-token")
    var draft = LogDraft()
    draft.body = "Read"
    ...
    XCTAssertEqual(
      URLProtocolStub.lastRequest?.value(forHTTPHeaderField: "Authorization"), "Bearer test-token")
  ...
  func testPathsAPIUsesAuthenticatedMutationContract() async throws {
    let id = UUID()
    let response =
      "{\"id\":\"\(id.uuidString)\",\"name\":\"Algorithms\",\"description\":\"Study\",\"status\":\"ACTIVE\",\"color\":\"#3B82F6\",\"activityLabel\":\"today\"}"
    URLProtocolStub.responseData = Data(response.utf8)
    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [URLProtocolStub.self]
    let api = PathsAPI(
  ...
  func testReportsThemeTextAndControlContrastInBothAppearances() {
    let light = (
      background: "f7f8fa", surface: "ffffff", selected: "e7ebf0", text: "252b36", muted: "606b7b",
      danger: "a12727", accent: "334155", onAccent: "ffffff"
    )
    let dark = (
      background: "151a22", surface: "1c2430", selected: "303e52", text: "e1e6ee", muted: "a7b2c2",
      danger: "ffaaaa", accent: "c4d1e2", onAccent: "18212e"
    )
    ...
    for (dark, background) in [(false, "f7f8fa"), (true, "151a22")] {
      for swatch in WorkspaceTheme.palette {
        ...
        XCTAssertGreaterThanOrEqual(
          contrastRatio(chartHex, background), 3, "\(swatch) in \(dark ? "dark" : "light")")
      }
    }
  }
  ...
  func testReportsThemeNonTextControlsGridlinesFocusAndSelectedIndicatorsMeetContrast() {
    let light = (
      background: "f7f8fa", surface: "ffffff", border: "7c8796", control: "7c8796", focus: "334155"
    )
    let dark = (
      background: "151a22", surface: "1c2430", border: "697789", control: "697789", focus: "c4d1e2"
    )
  ...
  func testAuthenticatedUITestingFixtureSkipsNetworkRefresh() async {
    let model = AppModel(
      api: APIClient(base: URL(string: "https://example.test/api/v1")!),
      arguments: ["Know", "-ui-testing", "-ui-testing-authenticated"])
  ...
  func testAPIClientDecodesResponsesAndAddsBearerToken() async throws {
    let entityId = UUID()
    URLProtocolStub.responseData =
      "{\"id\":\"\(entityId.uuidString)\",\"name\":\"Algorithms\",\"description\":null,\"status\":\"ACTIVE\"}"
      .data(using: .utf8)!
    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [URLProtocolStub.self]
  ...
  func testCalendarAPIUsesScopedAuthenticatedContracts() async throws {
    ...
    let api = CalendarAPI(
      client: APIClient(
        base: URL(string: "https://example.test/api/v1")!,
        session: URLSession(configuration: configuration)), token: "calendar-token")
    let id = UUID(uuidString: "00000000-0000-4000-8000-000000000020")!

    URLProtocolStub.responseData = Data(
      "[{\"id\":\"\(id)\",\"name\":\"Leave\",\"color\":null,\"scopes\":[\"CALENDAR\"]}]".utf8)
    ...
    XCTAssertEqual(
      URLProtocolStub.lastRequest?.url?.query, "startDate=2026-09-01&endDate=2026-09-30")

    URLProtocolStub.responseData = Data(
      "{\"date\":\"2026-09-03\",\"note\":null,\"labels\":[]}".utf8)
    _ = try await api.saveDay(
      date: "2026-09-03",
      request: CalendarDayRequest(
        note: nil, labels: [CalendarDayAssignment(labelId: id, portion: 0.5)]))
    XCTAssertEqual(URLProtocolStub.lastRequest?.httpMethod, "PUT")
    XCTAssertEqual(URLProtocolStub.lastRequest?.url?.path, "/api/v1/calendar/days/2026-09-03")
    let dayBody = try requestJSONObject()
    XCTAssertTrue(dayBody["note"] is NSNull)
    XCTAssertEqual(
      ((dayBody["labels"] as? [[String: Any]])?.first?["portion"] as? NSNumber)?.doubleValue, 0.5)

    URLProtocolStub.responseData = Data("[]".utf8)
    _ = try await api.saveRange(
      request: CalendarRangeRequest(
        startDate: "2026-09-01", endDate: "2026-09-03", note: "Away", labels: []))
    XCTAssertEqual(URLProtocolStub.lastRequest?.httpMethod, "PUT")
    XCTAssertEqual(URLProtocolStub.lastRequest?.url?.path, "/api/v1/calendar/days/range")
    let rangeBody = try requestJSONObject()
    XCTAssertEqual(rangeBody["startDate"] as? String, "2026-09-01")
    XCTAssertEqual(rangeBody["endDate"] as? String, "2026-09-03")
  }
  ...
  func testAPIClientDoesNotRetryNonIdempotentRequests() async throws {
    ...
    do {
      let _: Path = try await client.request(
        "/calendar/labels", method: "POST", body: Data("{}".utf8))
      XCTFail("Expected an offline error")
    } catch let error as APIError {
      if case .offline = error {} else { XCTFail("Expected offline, got \(error)") }
    }
