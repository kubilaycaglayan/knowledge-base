...
private actor LogsStub: LogsTransport {
  ...
  init() {
    values = [
      Log(
        id: id, body: "Original", occurredAt: "2026-09-11T11:30:00Z", labelIds: [],
        createdAt: "2026-09-11T11:30:00Z", updatedAt: "2026-09-11T11:30:00Z", version: 2)
    ]
  }
  ...
  func labels() async throws -> [LogLabel] {
    [LogLabel(id: labelID, name: "Important", color: "#2878D5", scopes: ["LOG"])]
  }
  func create(_ draft: LogDraft) async throws -> Log {
    let log = Log(
      id: UUID(), body: draft.body.trimmingCharacters(in: .whitespacesAndNewlines),
      occurredAt: LogFormatting.iso(draft.occurredAt), labelIds: [],
      createdAt: LogFormatting.iso(Date()), updatedAt: LogFormatting.iso(Date()), version: 0)
    values.append(log)
    return log
  }
  ...
  func update(id: UUID, draft: LogDraft, version: Int) async throws -> Log {
    ...
    if conflict && updateCount == 1 {
      throw APIError.http(status: 409, message: "Log changed in another window")
    }
    let old = try await fetch(id: id)
    let result = Log(
      id: id, body: draft.body, occurredAt: LogFormatting.iso(draft.occurredAt),
      labelIds: old.labelIds, createdAt: old.createdAt, updatedAt: old.updatedAt,
      version: version + 1)
  ...
  func updateLabels(id: UUID, ids: [UUID]) async throws -> Log {
    ...
    let old = try await fetch(id: id)
    return Log(
      id: id, body: old.body, occurredAt: old.occurredAt, labelIds: ids, createdAt: old.createdAt,
      updatedAt: old.updatedAt, version: old.version + 1)
  }
...
@MainActor final class LogsTests: XCTestCase {
  func testTimestampParsingAndSorting() async {
    ...
    let stub = LogsStub()
    let model = LogsModel(transport: stub)
    await model.load()
  ...
  func testGroupsRespectLocalMondayAndHourBoundaries() {
    ...
    XCTAssertEqual(
      LogFormatting.group("2026-09-14T07:00:00Z", now: now, calendar: calendar), "Today")
    XCTAssertEqual(
      LogFormatting.group("2026-09-13T10:00:00Z", now: now, calendar: calendar), "Yesterday")
    XCTAssertEqual(
      LogFormatting.group("2026-09-08T10:00:00Z", now: now, calendar: calendar), "Last week")
    XCTAssertTrue(
      LogFormatting.sameHour("2026-09-11T11:10:00Z", "2026-09-11T11:50:00Z", calendar: calendar))
    XCTAssertFalse(
      LogFormatting.sameHour("2026-09-11T11:10:00Z", "2026-09-11T12:00:00Z", calendar: calendar))
  }
  ...
  func testEveryLogCalendarGroupBoundary() {
    ...
    XCTAssertEqual(
      LogFormatting.group("2026-09-16T08:45:00Z", now: now, calendar: calendar), "Last hour")
    XCTAssertEqual(
      LogFormatting.group("2026-09-14T12:00:00Z", now: now, calendar: calendar), "This week")
    XCTAssertEqual(
      LogFormatting.group("2026-09-08T12:00:00Z", now: now, calendar: calendar), "Last week")
    XCTAssertEqual(
      LogFormatting.group("2026-09-02T12:00:00Z", now: now, calendar: calendar), "This month")
    XCTAssertEqual(
      LogFormatting.group("2026-08-15T12:00:00Z", now: now, calendar: calendar), "Last month")
    XCTAssertEqual(
      LogFormatting.group("2026-07-15T12:00:00Z", now: now, calendar: calendar), "July 2026")
  }

  func testCreateTrimsAndKeepsChangedDraftDuringRequest() async {
    let stub = LogsStub()
    let model = LogsModel(transport: stub)
    model.draft.body = "  Thought  "
    await model.create()
  ...
  func testConflictFetchesLatestVersionAndRetriesSnapshot() async {
    let stub = LogsStub()
    await stub.setConflict(true)
    let model = LogsModel(transport: stub)
    await model.load()
    model.beginEdit(model.logs[0])
    model.editDraft.body = "Local draft"
  ...
  func testFailedEditRetainsDraftForRetry() async {
    ...
    await model.load()
    model.beginEdit(model.logs[0])
    model.editDraft.body = "Keep this text"
...
extension LogsStub {
  fileprivate func setConflict(_ value: Bool) { conflict = value }
  fileprivate func setUpdateFailure(_ value: APIError) { updateFailure = value }
  fileprivate func setRemoveFailure(_ value: APIError) { removeFailure = value }
  fileprivate func setLoadFailure(_ value: APIError) { loadFailure = value }
}
