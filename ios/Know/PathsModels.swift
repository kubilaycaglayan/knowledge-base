...
struct PathsAPI: PathsTransport {
  ...
  func summary(id: UUID) async throws -> PathSummary {
    try await client.request("/paths/\(id)/summary", token: token)
  }

  func create(name: String, description: String?, color: String) async throws -> Path {
    let body = try JSONEncoder().encode(
      PathRequest(name: name, description: description, color: color))
  ...
  func remove(id: UUID) async throws {
    try await client.empty("/paths/\(id)", method: "DELETE", token: token)
  }
  func restore(id: UUID) async throws {
    try await client.empty("/paths/\(id)/restore", method: "POST", token: token)
  }
...
@MainActor @Observable final class PathsModel {
  ...
  init(
    transport: PathsTransport, unauthorized: @escaping () -> Void = {},
  ...
  func create(name: String, description: String, color: String) async -> Bool {
    ...
    defer { busy = false }
    do {
      let value = try await transport.create(
        name: cleanName, description: clean(description), color: color)
      paths.insert(value, at: 0)
      invalidateReports()
      return true
    } catch {
      fail(error, "Could not create the path.")
      return false
    }
  }
  ...
  func update(_ path: Path, name: String, description: String, color: String) async -> Bool {
    ...
    defer { busy = false }
    do {
      let value = try await transport.update(
        id: path.id, name: cleanName, description: clean(description), color: color)
      replace(value)
      if summaries[path.id] != nil { await loadSummary(for: value) }
      invalidateReports()
      return true
    } catch {
      fail(error, "Could not update path.")
      return false
    }
  }
  ...
  func remove(_ path: Path) async -> Bool {
    ...
    do {
      ...
      fail(error, "Could not remove path.")
      return false
    }
  }
  ...
  func undoRemoval() async -> Bool {
    ...
    do {
      ...
      fail(error, "Could not undo path removal.")
      return false
    }
  }
  ...
  func merge(source: Path, into target: Path) async -> Bool {
    ...
    do {
      ...
      fail(error, "Could not merge paths. Try again.")
      return false
    }
  }

  func session(id: UUID) async -> TrackedSession? {
    do { return try await transport.session(id: id) } catch {
      fail(error, "Could not load this session.")
      return nil
    }
  }
  ...
  private func expireRemoval() {
    pendingRemoval = nil
    removalTask = nil
  }

  private func fail(_ failure: Error, _ message: String) {
    if let failure = failure as? APIError, case .unauthorized = failure {
      unauthorized()
      return
    }
