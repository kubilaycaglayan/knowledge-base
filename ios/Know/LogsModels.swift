...
struct LogDraft: Equatable {
  ...
  init(_ log: Log) {
    body = log.body
    occurredAt = LogFormatting.date(log.occurredAt) ?? Date()
  }
}
...
enum LogFormatting {
  ...
  static func date(_ value: String?) -> Date? {
    ...
    return fractionalISO8601Formatter.date(from: value)
      ?? internetISO8601Formatter.date(from: value)
  }
  ...
  static func group(_ value: String, now: Date = Date(), calendar: Calendar = .current) -> String {
    ...
    if date >= month && date < calendar.date(byAdding: .month, value: 1, to: month)! {
      return "This month"
    }
    if date >= calendar.date(byAdding: .month, value: -1, to: month)! && date < month {
      return "Last month"
    }
    let formatter = DateFormatter()
    formatter.calendar = calendar
  ...
  static func sameDay(_ a: String, _ b: String, calendar: Calendar = .current) -> Bool {
    guard let a = date(a), let b = date(b) else { return false }
    return calendar.isDate(a, inSameDayAs: b)
  }
  static func sameHour(_ a: String, _ b: String, calendar: Calendar = .current) -> Bool {
    guard let a = date(a), let b = date(b) else { return false }
    return sameDay(a: a, b: b, calendar: calendar)
      && calendar.component(.hour, from: a) == calendar.component(.hour, from: b)
  }
  private static func sameDay(a: Date, b: Date, calendar: Calendar) -> Bool {
...
struct LogsAPI: LogsTransport {
  ...
  func labels() async throws -> [LogLabel] {
    try await client.request("/labels?scope=LOG", token: token)
  }
  private func body(_ draft: LogDraft, version: Int? = nil) throws -> Data {
    var fields: [String: Any] = [
      "body": draft.body, "occurredAt": LogFormatting.iso(draft.occurredAt),
    ]
    if let version { fields["version"] = version }
    return try JSONSerialization.data(withJSONObject: fields)
  }
  func create(_ draft: LogDraft) async throws -> Log {
    try await client.request("/logs", method: "POST", body: body(draft), token: token)
  }
  func fetch(id: UUID) async throws -> Log { try await client.request("/logs/\(id)", token: token) }
  func update(id: UUID, draft: LogDraft, version: Int) async throws -> Log {
    try await client.request(
      "/logs/\(id)", method: "PUT", body: body(draft, version: version), token: token)
  }
  func updateLabels(id: UUID, ids: [UUID]) async throws -> Log {
    try await client.request(
      "/logs/\(id)/labels", method: "PUT",
      body: JSONSerialization.data(withJSONObject: ["labelIds": ids.map(\.uuidString)]),
      token: token)
  }
  func remove(id: UUID) async throws {
    try await client.empty("/logs/\(id)", method: "DELETE", token: token)
  }
}
