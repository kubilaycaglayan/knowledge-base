...
struct WorkspaceView: View {
  ...
  private static func accountID(_ token: String?) -> String {
    ...
    var base64 = String(part).replacingOccurrences(of: "-", with: "+").replacingOccurrences(
      of: "_", with: "/")
  ...
  var body: some View {
    VStack(spacing: 0) {
      VStack(alignment: .leading, spacing: 8) {
        ...
        ScrollView(.horizontal, showsIndicators: false) {
          HStack(spacing: 2) {
            ForEach(
              ["Sessions", "Logs", "Labels", "Notes", "Paths", "Calendar", "Reports"], id: \.self
            ) { name in
              Button {
                section = name
              } label: {
    ...
    .task {
      if uiTesting {
        ...
        await labels.load()
        await notes.load()
        await notes.loadLabels()
        await paths.load()
        await calendar.load()
      } else {
        resumeVisibleSection()
      }
    }
    .onChange(of: phase) { _, phase in
      if phase == .active {
        if !uiTesting {
          resumeVisibleSection()
          Task { await reports.load(force: true) }
        }
      } else {
        sessions.suspend()
        logs.suspend()
    ...
    .onChange(of: section) { _, value in
      if !uiTesting { resumeVisibleSection() }
      if value != "Sessions" { Task { await app.refresh() } }
    }
    .onDisappear {
      sessions.suspend()
      logs.suspend()
    ...
    .confirmationDialog(
      "Discard unsaved changes and sign out?", isPresented: $signOutConfirmation,
  ...
  private func resumeVisibleSection() {
    ...
    if section == "Sessions" {
      sessions.resume(client: app.api, token: token)
    } else {
      sessions.suspend()
    }
