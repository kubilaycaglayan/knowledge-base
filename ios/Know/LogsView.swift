...
struct LogsView: View {
    ...
    var body: some View {
        ...
        }.background(WorkspaceTheme.background(scheme)).task { while !Task.isCancelled { try? await Task.sleep(for: .seconds(1)); now = Date(); if followsClock { changingClock = true; model.draft.occurredAt = LogFormatting.deviceMinute(Date()); changingClock = false } } }
