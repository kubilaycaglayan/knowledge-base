...
struct PathsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                if let removed = model.pendingRemoval { HStack { Text("Removed “\(removed.name)”."); Spacer(); Button("Undo") { Task { await model.undoRemoval() } } }.padding(12).background(WorkspaceTheme.selected(scheme), in: RoundedRectangle(cornerRadius: 6)).accessibilityElement(children: .combine).accessibilityLabel("Path removed. Undo available for 8 seconds.").accessibilityIdentifier("paths.undo") }
        ...
        .sheet(isPresented: $addOpen) { pathEditor(title: "Add a path", path: nil, submit: saveNew, cancel: { addOpen = false }) }
        .sheet(item: $editPath) { path in pathEditor(title: "Edit path", path: path, submit: { await saveEdit(path) }, cancel: { editPath = nil }) }
        .sheet(item: $mergeSource) { source in MergePathSheet(source: source, paths: model.paths) { target in mergeConfirmation = MergeSelection(source: source, target: target) } }
        ...
        .confirmationDialog("Discard unsaved path changes?", isPresented: $discardEdit, titleVisibility: .visible) {
        ...
        .confirmationDialog("Merge paths?", isPresented: Binding(get: { mergeConfirmation != nil }, set: { if !$0 { mergeConfirmation = nil } })) {
            ...
            Button("Cancel", role: .cancel) {}
        } message: { Text(mergeConfirmation.map { "Merge \($0.source.name) into \($0.target.name)? All sessions will move and \($0.source.name) will be removed." } ?? "") }
    }
    ...
    @ViewBuilder private func descriptionView(_ value: String) -> some View {
        if let start = value.range(of: "http://") ?? value.range(of: "https://") {
            ...
            if let url = URL(string: clean), url.scheme == "http" || url.scheme == "https" { Link(value, destination: url).foregroundStyle(WorkspaceTheme.text(scheme)) } else { Text(value).fixedSize(horizontal: false, vertical: true) }
    ...
    private func history(_ path: Path) -> some View {
        NavigationStack { ScrollView { VStack(alignment: .leading, spacing: 16) { if let summary = model.summaries[path.id] { Text("\(SessionFormatting.duration(summary.trackedSeconds)) tracked").foregroundStyle(WorkspaceTheme.muted(scheme)); let groups = historyGroups(summary.recentActivity); if groups.isEmpty { Text("No recent activity yet.").foregroundStyle(WorkspaceTheme.muted(scheme)) } else { ForEach(groups) { group in Section { ForEach(group.events) { event in historyEvent(event) } } header: { Text(group.title).font(.headline).foregroundStyle(WorkspaceTheme.text(scheme)) } } } } else if let error = model.error { VStack(alignment: .leading, spacing: 10) { Text(error).foregroundStyle(WorkspaceTheme.danger(scheme)); Button("Retry") { Task { await model.loadSummary(for: path) } }.buttonStyle(WorkspaceButton()) } } else { ProgressView("Loading history…") } }.padding(16) }.accessibilityAddTraits(.isModal).navigationTitle(path.name).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Close") { historyPath = nil }.accessibilityIdentifier("paths.history.close") } }.task { if model.summaries[path.id] == nil { await model.loadSummary(for: path) } } }
    }

    @ViewBuilder private func historyEvent(_ event: Activity) -> some View { VStack(alignment: .leading, spacing: 8) { Text(PathHistoryFormatting.display(event.occurredAt)).font(.caption).foregroundStyle(WorkspaceTheme.muted(scheme)); if event.title.hasPrefix("Tracked "), let seconds = Int64(event.title.split(separator: " ").dropFirst().first ?? "0") { Text(SessionFormatting.duration(seconds)).font(.headline) }; if let detail = event.detail { Text(detail).fixedSize(horizontal: false, vertical: true) }; if let ids = event.labelIds, !ids.isEmpty { WorkspaceFlow { ForEach(ids, id: \.self) { id in WorkspaceChip(text: sessions.labels.first { $0.id == id }?.name ?? "Removed label") } }.accessibilityLabel("Session labels") }; if let id = event.timeEntryId { Button("Edit session") { Task { editingSession = await model.session(id: id) } }.accessibilityIdentifier("paths.history.edit.\(id)") } }.padding(12).frame(maxWidth: .infinity, alignment: .leading).background(WorkspaceTheme.surface(scheme), in: RoundedRectangle(cornerRadius: 6)).overlay(RoundedRectangle(cornerRadius: 6).stroke(WorkspaceTheme.border(scheme))) }

    private func historyGroups(_ events: [Activity]) -> [HistoryGroup] {
        let visible = PathHistoryFormatting.visible(events).sorted { (PathHistoryFormatting.date($0.occurredAt) ?? .distantPast) > (PathHistoryFormatting.date($1.occurredAt) ?? .distantPast) }
        var result: [HistoryGroup] = []
        for event in visible { let title = PathHistoryFormatting.group(event.occurredAt); if let index = result.firstIndex(where: { $0.title == title }) { result[index].events.append(event) } else { result.append(HistoryGroup(title: title, events: [event])) } }
        return result
    }

    private func errorNotice(_ value: String) -> some View { HStack(spacing: 12) { Text(value).font(.subheadline).foregroundStyle(WorkspaceTheme.danger(scheme)).accessibilityLabel(value); Button("Retry") { Task { await model.load(force: true) } }.buttonStyle(WorkspaceButton()).accessibilityIdentifier("paths.retry") }.accessibilityIdentifier("paths.error") }
...
private struct HistoryGroup: Identifiable { let id = UUID(); let title: String; var events: [Activity] }
private struct MergeSelection: Identifiable { let id = UUID(); let source: Path; let target: Path }
