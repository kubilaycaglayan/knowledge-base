...
struct CalendarFixture: CalendarTransport {
  ...
  func labels() async throws -> [KBLabel] {
    ...
    return [
      KBLabel(id: labelID, name: "Milestone", color: WorkspaceTheme.palette[0], scopes: [.calendar])
    ]
  }
  func days(startDate: String, endDate: String) async throws -> [CalendarDay] {
    if arguments.contains("-calendar-loading") { try await Task.sleep(nanoseconds: 5_000_000_000) }
    if arguments.contains("-calendar-unauthorized") { throw APIError.unauthorized }
    if arguments.contains("-calendar-offline") || arguments.contains("-calendar-load-error") {
      throw APIError.offline
    }
    guard arguments.contains("-calendar-saved-day") || arguments.contains("-calendar-range") else {
      return []
    }
    return [
      CalendarDay(
        date: dayDate, note: "A useful day",
        labels: [
          CalendarAssignment(
            labelId: labelID, name: "Milestone", color: WorkspaceTheme.palette[0],
            portion: arguments.contains("-calendar-range") ? 0.50 : nil)
        ])
    ]
  }
  func saveDay(date: String, request: CalendarDayRequest) async throws -> CalendarDay {
    if arguments.contains("-calendar-mutation-error") || arguments.contains("-calendar-offline") {
      throw APIError.offline
    }
    return CalendarDay(
      ...
      labels: request.labels.map {
        CalendarAssignment(
          labelId: $0.labelId, name: "Milestone", color: WorkspaceTheme.palette[0],
          portion: $0.portion)
      })
  }
  func saveRange(request: CalendarRangeRequest) async throws -> [CalendarDay] {
    ...
    guard let start = CalendarDate.date(request.startDate),
      let end = CalendarDate.date(request.endDate)
    else { return [] }
    ...
    while date <= end {
      result.append(
        CalendarDay(
          date: CalendarDate.string(date, calendar: calendar), note: request.note,
          labels: request.labels.map {
            CalendarAssignment(
              labelId: $0.labelId, name: "Milestone", color: WorkspaceTheme.palette[0],
              portion: $0.portion)
  ...
  func createLabel(name: String, color: String) async throws -> KBLabel {
    if arguments.contains("-calendar-mutation-error") {
      throw APIError.http(status: 409, message: nil)
    }
    return KBLabel(id: UUID(), name: name, color: color, scopes: [.calendar])
  }
  func updateLabel(_ label: KBLabel) async throws -> KBLabel {
    if arguments.contains("-calendar-mutation-error") {
      throw APIError.http(status: 503, message: nil)
    }
    return label
  }
}
