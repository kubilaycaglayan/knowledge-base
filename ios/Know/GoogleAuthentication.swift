...
import SwiftUI
  ...
  struct NativeGoogleButton: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> GIDSignInButton {
      ...
      button.addTarget(
        context.coordinator, action: #selector(Coordinator.signIn), for: .touchUpInside)
      return button
    }
  ...
  struct NativeGoogleButton: View {
    ...
    var body: some View {
      GoogleSignInButton(
        scheme: dark ? .dark : .light, style: .wide, state: isEnabled ? .normal : .disabled,
        action: action)
    }
  }
#endif
...
enum SessionError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .configuration:
      return
        "Google sign-in is not available in this build. You can sign in with email and password."
...
@MainActor enum GoogleAuthentication {
  static func configurationIsValid(clientID: String?, serverID: String?, urlSchemes: [String])
    -> Bool
  {
  ...
  static func idToken() async throws -> String {
    guard let clientID = Bundle.main.object(forInfoDictionaryKey: "GIDClientID") as? String,
      let serverID = Bundle.main.object(forInfoDictionaryKey: "GIDServerClientID") as? String,
      isConfigured
    else { throw SessionError.configuration }
    let callback = clientID.split(separator: ".").reversed().joined(separator: ".")
    let urlTypes =
      Bundle.main.object(forInfoDictionaryKey: "CFBundleURLTypes") as? [[String: Any]] ?? []
    guard
      urlTypes.contains(where: { ($0["CFBundleURLSchemes"] as? [String] ?? []).contains(callback) })
    else {
