...
@MainActor @Observable final class SessionsModel {
  ...
  private func applySelection(_ selection: TrackerSelection, submitted: SessionDraft? = nil) {
    ...
    if draft.pathId == pathBase {
      draft.pathId = selection.pathId
      nextBaseline.pathId = selection.pathId
    }
    let labelsBase = submitted?.labelIds ?? selectionBaseline.labelIds
    if draft.labelIds == labelsBase {
      draft.labelIds = selection.labelIds
      nextBaseline.labelIds = selection.labelIds
    }
  ...
  init(
    transport: SessionsTransport, defaults: UserDefaults? = .standard, account: String = "default",
    ...
    recentPathIds = (defaults?.stringArray(forKey: recentKey) ?? []).compactMap(
      UUID.init(uuidString:))
  }

  var activePaths: [Path] { paths.filter { $0.status == "ACTIVE" } }
  var recentPaths: [Path] {
    recentPathIds.compactMap { id in activePaths.first { $0.id == id } }.prefix(5).map { $0 }
  }
  ...
  func loadHistory(page: Int = 0) async {
    ...
    do {
      ...
      if result.sessions.isEmpty && result.page > 0 {
        result = try await transport.history(
          page: max(0, min(result.page - 1, result.totalPages - 1)))
      }
  ...
  func apply(_ snapshot: TrackedSession?) {
    ...
    if let current {
      ...
      if let previous, previous.id == current.id {
        ...
        if descriptionFocused || pending.description != baseline.description {
          draft.description = pending.description
        }
  ...
  func toggleTimer() async {
    ...
    do {
      ...
      fail(
        error, "Could not update the timer. Only one timer can run at a time. Retry after syncing.")
    }
  }
  ...
  func saveTimer() async {
    ...
    do {
      ...
      fail(error, "Could not save the active timer settings. Your edits are kept; try again.")
    }
  }
  ...
  func createPath(_ name: String) async -> Bool {
    ...
    do {
      ...
      busy = false
      fail(error, "Could not create path. Try again.")
      return false
    }
  }
  ...
  func createLabel(_ name: String) async -> Bool {
    ...
    do {
      ...
      busy = false
      fail(error, "Could not create the session label. Try again.")
      return false
    }
  }
  ...
  func save(_ session: TrackedSession, draft: SessionDraft) async -> Bool {
    ...
    do {
      ...
      fail(error, "Could not update this session. Check its time range and selections.")
      return false
    }
  }
  ...
  private func fail(_ failure: Error, _ message: String) {
    ...
    error =
      (failure as? APIError).map {
        if case .offline = $0 { return "No network connection. Reconnect and try again." }
        return message
      } ?? message
  }
