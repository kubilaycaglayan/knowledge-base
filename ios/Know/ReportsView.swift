...
struct ReportsView: View {
    ...
    private var controls: some View { VStack(alignment: .leading, spacing: 10) {
        ...
        Picker("Aggregation", selection: Binding(get: { model.query.aggregation }, set: { value in Task { await model.setAggregation(value) } })) { ForEach(ReportAggregation.allCases) { Text($0.title).tag($0) } }.pickerStyle(.segmented).padding(.vertical, 7).accessibilityLabel("Report aggregation").accessibilityValue(model.query.aggregation.title).accessibilityHint("Changes how report time is grouped.").accessibilityIdentifier("reports.aggregation")
