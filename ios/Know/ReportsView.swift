...
struct ReportsView: View {
    ...
    private var sankeyControl: some View { Group { Button(model.showSankey ? "Show bar chart" : "Show Sankey") { model.showSankey.toggle() }.frame(minHeight: 44).workspaceFocusRing().accessibilityHint("Switches between the activity chart and time flow.").accessibilityIdentifier("reports.sankey.toggle"); if model.showSankey { sankey } } }
    private var controls: some View { VStack(alignment: .leading, spacing: 10) {
        HStack { Button("Previous") { Task { await model.shift(-1) } }.padding(.vertical, 12).contentShape(Rectangle()).workspaceFocusRing().accessibilityHint("Moves to the previous report period."); Button("Next") { Task { await model.shift(1) } }.padding(.vertical, 12).contentShape(Rectangle()).workspaceFocusRing().accessibilityHint("Moves to the next report period."); Spacer(); Menu("Preset") { ForEach(ReportDateMath.presets, id: \.self) { preset in Button(preset) { Task { await model.setPreset(preset) } } } }.frame(minHeight: 44).workspaceFocusRing().accessibilityLabel("Report preset").accessibilityHint("Changes the report date range.").accessibilityIdentifier("reports.preset") }
        aggregationControl
        HStack { dateButton(start: true); dateButton(start: false) }.frame(minHeight: 44).sheet(item: $editingDate) { field in
            NavigationStack {
                DatePicker(field == .start ? "Start date" : "End date", selection: $dateDraft, displayedComponents: .date)
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                ForEach(Array(values.enumerated()), id: \.element.identity) { index, value in
                    HStack { Circle().fill(WorkspaceTheme.chartColor(value.color ?? WorkspaceTheme.palette[index % WorkspaceTheme.palette.count], scheme)).frame(width: 10, height: 10); Text(value.label).lineLimit(2); Spacer(); Text(duration(value.seconds)).monospacedDigit() }.padding(.vertical, 6)
                }
            }
    ...
    @ViewBuilder private func donut(values: [ReportCategory], total: Int64) -> some View {
        ZStack {
            ...
            if total > 0 { ForEach(Array(values.enumerated()), id: \.element.identity) { index, value in Circle().trim(from: donutStart(values, index: index, total: total), to: donutEnd(values, index: index, total: total)).stroke(WorkspaceTheme.chartColor(value.color ?? WorkspaceTheme.palette[index % WorkspaceTheme.palette.count], scheme), lineWidth: 14).rotationEffect(.degrees(-90)) } }
