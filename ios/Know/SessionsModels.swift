...
enum SessionFormatting {
  ...
  static func group(_ timestamp: String, now: Date = Date(), calendar: Calendar = .current)
    -> String
  {
    ...
    if calendar.isDate(
      date, equalTo: calendar.date(byAdding: .month, value: -1, to: month)!, toGranularity: .month)
    {
      return "Last month"
    }
...
struct SessionsAPI: SessionsTransport {
  ...
  func history(page: Int) async throws -> SessionPage {
    try await client.request("/time-entries?page=\(page)&size=50", token: token)
  }
  func current() async throws -> TrackedSession? {
    try await client.optional("/timers/current", token: token)
  }
  func selection() async throws -> TrackerSelection {
    try await client.request("/timers/draft", token: token)
  }
  func saveSelection(_ draft: SessionDraft) async throws -> TrackerSelection {
    let selection = TrackerSelection(
      pathId: draft.pathId, labelIds: draft.labelIds, description: draft.description)
    return try await client.request(
      "/timers/draft", method: "PUT", body: JSONEncoder().encode(selection), token: token)
  }
  func start(_ draft: SessionDraft) async throws -> TrackedSession {
    var fields =
      try JSONSerialization.jsonObject(with: draft.body(completed: false)) as! [String: Any]
    fields.removeValue(forKey: "startedAt")
    fields["source"] = "IOS"
    return try await client.request(
      "/timers", method: "POST", body: JSONSerialization.data(withJSONObject: fields), token: token)
  }
  func updateTimer(id: UUID, draft: SessionDraft) async throws -> TrackedSession {
    try await client.request(
      "/timers/\(id)", method: "PUT", body: draft.body(completed: false), token: token)
  }
  func stop(id: UUID) async throws {
    try await client.empty("/timers/\(id)/stop", method: "POST", token: token)
  }
  func save(id: UUID, draft: SessionDraft) async throws {
    let _: TrackedSession = try await client.request(
      "/time-entries/\(id)", method: "PUT", body: draft.body(completed: true), token: token)
  }
  func remove(id: UUID) async throws {
    try await client.empty("/time-entries/\(id)", method: "DELETE", token: token)
  }
  func createPath(name: String) async throws -> Path {
    try await client.request(
      "/paths", method: "POST",
      body: JSONSerialization.data(withJSONObject: [
        "name": name, "description": NSNull(), "color": "#EF4444",
      ]), token: token)
  }
  func createLabel(name: String) async throws -> SessionLabel {
    try await client.request(
