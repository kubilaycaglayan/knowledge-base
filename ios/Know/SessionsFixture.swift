...
actor SessionsFixture: SessionsTransport {
    ...
    private var allPaths = [Path(id: pathID, name: "Distributed systems", description: nil, status: "ACTIVE", color: "#2188FF")]
    private var allLabels = [SessionLabel(id: labelID, name: "Study day", color: "#2878D5", scopes: ["TIME_ENTRY"])]
    ...
    func paths() async throws -> [Path] { if fails { fails = false; throw APIError.offline }; return allPaths }
    func labels() async throws -> [SessionLabel] { allLabels }
    func history(page: Int) async throws -> SessionPage { SessionPage(sessions: entries, page: 0, totalPages: entries.isEmpty ? 0 : 1, totalSessions: entries.count) }
    func current() async throws -> TrackedSession? { timer }
    func start(_ draft: SessionDraft) async throws -> TrackedSession {
        let session = TrackedSession(id: UUID(), pathId: draft.pathId, labelIds: draft.labelIds, startedAt: SessionFormatting.iso(Date()), description: draft.description, source: "IOS", running: true)
        timer = session
        return session
    }
    func updateTimer(id: UUID, draft: SessionDraft) async throws -> TrackedSession {
        let session = TrackedSession(id: id, pathId: draft.pathId, labelIds: draft.labelIds, startedAt: SessionFormatting.iso(draft.startedAt), description: draft.description, source: "IOS", running: true)
        timer = session
        return session
    }
    ...
    func save(id: UUID, draft: SessionDraft) async throws {
        ...
        entries[index] = TrackedSession(id: id, pathId: draft.pathId, labelIds: draft.labelIds, startedAt: SessionFormatting.iso(draft.startedAt), endedAt: SessionFormatting.iso(draft.endedAt), durationSeconds: Int64(draft.endedAt.timeIntervalSince(draft.startedAt)), description: draft.description, source: draft.source, running: false)
    }
