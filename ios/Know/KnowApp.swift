...
struct Path: Codable, Identifiable {
  ...
  init(
    id: UUID, name: String, description: String?, status: String, color: String? = nil,
    activityLabel: String? = nil, createdAt: String? = nil, updatedAt: String? = nil
  ) {
    self.id = id
    self.name = name
...
struct Activity: Codable, Identifiable {
  ...
  let labelIds: [UUID]?
  init(
    id: UUID, type: String, title: String, detail: String?, occurredAt: String, timeEntryId: UUID?,
    labelIds: [UUID]? = nil
  ) {
    self.id = id
    self.type = type
...
struct PathRequest: Codable {
  ...
  let description: String?
  let color: String?
  init(name: String, description: String?, color: String? = nil) {
    self.name = name
    self.description = description
    self.color = color
  }
...
struct APIClient {
  ...
  private func makeRequest(
    ...
    components.path =
      base.path.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        .isEmpty
      ? relative.path : base.path + (relative.path.hasPrefix("/") ? "" : "/") + relative.path
  ...
  private func send(_ request: URLRequest) async throws -> Data {
    ...
    for attempt in 0..<attempts {
      do {
        ...
        return data
      } catch let error as APIError { throw error } catch let error as URLError
        where Self.isTransient(error)
      {
  ...
  func empty(_ path: String, method: String, body: Data = Data("{}".utf8), token: String)
    async throws
  {
...
@MainActor final class AppModel: ObservableObject {
  ...
  func authenticateWithGoogle(idToken: () async throws -> String) async {
    ...
    do {
      ...
      if generation == authGeneration {
        authError =
          (error as? SessionError)?.localizedDescription
  ...
  private func authenticationMessage(_ error: Error) -> String {
    ...
    if case SessionError.storage = error {
  ...
  func createLabel(name: String) async {
    ...
    do {
      ...
      let _: DailyLabel = try await api.request(
        "/calendar/labels", method: "POST", body: body, token: token)
      await refresh()
    } catch {
