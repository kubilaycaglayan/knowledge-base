...
enum WorkspaceTheme {
  ...
  static func color(_ hex: String) -> Color {
    let value =
      UInt64(hex.trimmingCharacters(in: CharacterSet(charactersIn: "#")), radix: 16) ?? 0x64748B
    return Color(
      red: Double(value >> 16 & 255) / 255, green: Double(value >> 8 & 255) / 255,
      blue: Double(value & 255) / 255)
  }
  static func background(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "151a22" : "f7f8fa")
  }
  static func surface(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "1c2430" : "ffffff")
  }
  ...
  static func border(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "697789" : "7c8796")
  }
  static func control(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "697789" : "7c8796")
  }
  static func selected(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "303e52" : "e7ebf0")
  }
  static func accent(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "c4d1e2" : "334155")
  }
  static func onAccent(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "18212e" : "ffffff")
  }
  static func danger(_ scheme: ColorScheme) -> Color {
    color(scheme == .dark ? "ffaaaa" : "a12727")
  }
  static func chartColor(_ hex: String, _ scheme: ColorScheme) -> Color {
    color(accessibleChartHex(hex, scheme))
  }

  static func accessibleChartHex(_ hex: String, _ scheme: ColorScheme) -> String {
    accessibleChartHex(hex, dark: scheme == .dark)
  }
  ...
  static func accessibleChartHex(_ hex: String, dark: Bool) -> String {
    ...
    if !dark {
      ...
      for _ in 0..<24 {
        ...
        if contrastRatio(base.map { $0 * factor }, background) >= 3.1 {
          low = factor
        } else {
          high = factor
        }
  ...
  private static func luminance(_ components: [Double]) -> Double {
    let linear = components.map { $0 <= 0.03928 ? $0 / 12.92 : pow(($0 + 0.055) / 1.055, 2.4) }
    return 0.2126 * linear[0] + 0.7152 * linear[1] + 0.0722 * linear[2]
  }
  private static func contrastRatio(_ foreground: [Double], _ background: [Double]) -> Double {
    let a = luminance(foreground)
    let b = luminance(background)
    return (max(a, b) + 0.05) / (min(a, b) + 0.05)
  }
  private static func normalizedHex(_ components: [Double]) -> String {
    components.map {
      String(format: "%02X", Int(($0 * 255).rounded())).padding(
...
struct WorkspaceButton: ButtonStyle {
  ...
  func makeBody(configuration: Configuration) -> some View {
    configuration.label.font(.subheadline.weight(.medium)).padding(.horizontal, 12).frame(
      minHeight: 44
    )
    .foregroundStyle(primary ? WorkspaceTheme.onAccent(scheme) : WorkspaceTheme.text(scheme))
    .background(
      primary ? WorkspaceTheme.accent(scheme) : WorkspaceTheme.surface(scheme),
      in: RoundedRectangle(cornerRadius: 4)
    )
    .overlay(
      RoundedRectangle(cornerRadius: 4).stroke(
        primary ? WorkspaceTheme.accent(scheme) : WorkspaceTheme.control(scheme))
    )
...
struct WorkspaceChip: View {
  ...
  var prominent = false
  var body: some View {
    Text(text).font(prominent ? .subheadline.weight(.semibold) : .caption).padding(.horizontal, 6)
      .padding(.vertical, 3)
      ...
      .overlay(
        RoundedRectangle(cornerRadius: 4).fill(
          color.map(WorkspaceTheme.color)?.opacity(0.14) ?? .clear)
      )
      .overlay(
        RoundedRectangle(cornerRadius: 4).stroke(
          color.map(WorkspaceTheme.color)?.opacity(0.4) ?? WorkspaceTheme.border(scheme))
      )
    ...
    for (index, point) in result.points.enumerated() {
      subviews[index].place(
        ...
        proposal: ProposedViewSize(
          width: min(bounds.width, subviews[index].sizeThatFits(.unspecified).width), height: nil))
    }
  }
  private func arrange(width: CGFloat, subviews: Subviews) -> (size: CGSize, points: [CGPoint]) {
    ...
    for view in subviews {
      let size = view.sizeThatFits(
        ProposedViewSize(width: min(width, view.sizeThatFits(.unspecified).width), height: nil))
