...
struct SessionsView: View {
  ...
  private var groups: [(id: UUID, label: String, sessions: [TrackedSession])] {
    ...
    for session in model.history.sessions {
      ...
      if result.last?.label == label {
        ...
        result.append((session.id, label, [session]))
      }
    }
    return result
  }
  ...
  var body: some View {
    ScrollView {
      LazyVStack(alignment: .leading, spacing: 24) {
        ...
        if model.loading && !model.loaded {
          ...
          Text("Start a session above to record your time.").font(.footnote).foregroundStyle(
        ...
        ForEach(groups, id: \.id) { group in
          HStack {
            ...
            Spacer(minLength: 12)
            Text(SessionFormatting.groupDuration(group.sessions)).monospacedDigit().foregroundStyle(
              WorkspaceTheme.muted(scheme))
          }
    ...
    .onChange(of: descriptionFocused) { _, focused in
      ...
      if !focused {
        Task {
          if model.descriptionNeedsSave { await model.saveTimer() } else { await model.sync() }
        }
      }
    }
    ...
    .confirmationDialog(
      ...
      Button("Remove session", role: .destructive) {
        if let session = removing { Task { await model.remove(session) } }
        removing = nil
      }
    ...
    VStack(alignment: .leading, spacing: 0) {
      HStack(spacing: 8) {
        ...
        if !model.draft.labelIds.isEmpty {
          WorkspaceChip(
            text: model.draft.labelIds.compactMap { model.labelsByID[$0]?.name }.joined(
              separator: ", ")
          )
        ...
        Button {
          ...
          .foregroundStyle(
            model.timer == nil
              ? WorkspaceTheme.color(scheme == .dark ? "87d5ae" : "197a43")
              : WorkspaceTheme.danger(scheme)
          )
          .background(
            model.timer == nil
              ? WorkspaceTheme.color(scheme == .dark ? "20372b" : "edf8f0")
              : WorkspaceTheme.color(scheme == .dark ? "3a242b" : "fff4f4"),
            in: RoundedRectangle(cornerRadius: 6)
          )
          .overlay(
            RoundedRectangle(cornerRadius: 6).stroke(
              model.timer == nil
                ? WorkspaceTheme.color("4f9b6d") : WorkspaceTheme.danger(scheme).opacity(0.5))
          )
      ...
      VStack(alignment: .leading, spacing: 12) {
        heading("Path")
        Menu {
          Button("＋ Add a new path…") {
            labelsExpanded = false
            addingPath = true
          }
          Divider()
          ForEach(model.activePaths) { path in
            Button(path.name) {
              labelsExpanded = false
              Task { await model.choosePath(path.id) }
            }
          }
        } label: {
          HStack {
            Text(model.draft.pathId.flatMap { model.pathsByID[$0] }?.name ?? "Choose a path…")
            Spacer()
            Image(systemName: "chevron.down").font(.caption)
          }
          .frame(maxWidth: .infinity, alignment: .leading).modifier(WorkspaceControl())
        }.buttonStyle(.plain).disabled(model.busy).accessibilityLabel("Timer path")
          .accessibilityIdentifier("timer.path")
        if !model.recentPaths.isEmpty {
          WorkspaceFlow {
            Text("Recent").font(.caption).foregroundStyle(WorkspaceTheme.muted(scheme)).frame(
              minHeight: 44)
            ForEach(model.recentPaths) { path in
              chipButton(path.name, selected: model.draft.pathId == path.id) {
                labelsExpanded = false
                Task { await model.choosePath(path.id) }
              }
            }
        ...
        HStack {
          ...
          Text(
            "\(model.labels.count) available\(selectedCount == 0 ? "" : " · \(selectedCount) selected")"
          )
          .font(.caption).foregroundStyle(WorkspaceTheme.muted(scheme)).accessibilityIdentifier(
            "timer.labels.summary")
          if !model.labels.isEmpty {
            Button {
              labelsExpanded.toggle()
            } label: {
              Image(systemName: labelsExpanded ? "chevron.up" : "chevron.down").font(.caption)
                .frame(width: 44, height: 44)
            }
            .buttonStyle(.plain)
        ...
        WorkspaceFlow {
          ForEach(orderedSessionLabels) { label in
            chipButton(label.name, selected: model.draft.labelIds.contains(label.id)) {
              Task {
                await model.toggleLabel(label.id)
                labelsExpanded = true
              }
            }
          ...
          if model.labels.isEmpty {
            Text("No session labels yet.").font(.caption).foregroundStyle(
              WorkspaceTheme.muted(scheme)
            ).frame(minHeight: 44)
          }
        }
        ...
        .padding(6).background(
          WorkspaceTheme.background(scheme), in: RoundedRectangle(cornerRadius: 6)
        )
        ...
        TextField("What are you working on…", text: $model.draft.description, axis: .vertical)
          ...
          .focused($descriptionFocused).accessibilityLabel("Timer description")
          .accessibilityIdentifier("timer.description")
          .onSubmit { Task { await model.saveTimer() } }
        if model.timer != nil && model.hasUnsavedDraft {
          Button {
            Task { await model.saveTimer() }
          } label: {
            busyLabel("Save timer settings")
          }
  ...
  private func heading(_ title: String, suffix: String = "") -> some View {
    HStack(spacing: 4) {
      Text(title.uppercased()).font(.caption2.weight(.semibold)).tracking(1)
      Text(suffix).font(.caption2)
    }
    .foregroundStyle(WorkspaceTheme.muted(scheme))
  }
  private func chipButton(_ title: String, selected: Bool, action: @escaping () -> Void)
    -> some View
  {
    Button(action: action) {
      HStack(spacing: 4) {
        Text(title)
        if selected { Image(systemName: "xmark").font(.caption2) }
      }
      ...
      .background(
        selected ? WorkspaceTheme.accent(scheme) : WorkspaceTheme.selected(scheme),
  ...
  private func sessionCard(_ session: TrackedSession) -> some View {
    VStack(alignment: .leading, spacing: 8) {
      if editing?.id == session.id {
        ...
        if let text = session.description, !text.isEmpty {
          Text(text).font(.subheadline).foregroundStyle(WorkspaceTheme.muted(scheme))
        }
        WorkspaceFlow(spacing: 10) {
          Button(session.running == true ? "Stop to edit" : "Edit session") { editing = session }
            .buttonStyle(WorkspaceButton()).disabled(
              session.running == true || model.busy || editing != nil
            )
          ...
          if session.running != true {
            Button {
              ...
              Text("Remove session").foregroundStyle(WorkspaceTheme.danger(scheme))
            }
            .buttonStyle(WorkspaceButton()).disabled(model.busy)
        ...
        WorkspaceFlow {
          WorkspaceChip(
            text: session.running == true
              ? "Running" : SessionFormatting.duration(session.durationSeconds ?? 0))
          WorkspaceChip(
            text:
              "\(session.source) · \(SessionFormatting.date(session.startedAt)?.formatted(date: .numeric, time: .standard) ?? session.startedAt)"
          )
        }.monospacedDigit()
      }
      ...
      .overlay(
        RoundedRectangle(cornerRadius: 4).stroke(WorkspaceTheme.control(scheme).opacity(0.6))
      )
      .compositingGroup()
