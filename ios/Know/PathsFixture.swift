...
actor PathsFixture: PathsTransport {
    ...
    init(arguments: [String] = ProcessInfo.processInfo.arguments) {
        ...
        values = [
            Path(id: Self.pathID, name: "Distributed systems", description: "Replication and consistency models", status: "ACTIVE", color: "#2188FF", activityLabel: "today"),
            Path(id: Self.otherID, name: "Writing", description: nil, status: "ACTIVE", color: "#8B5CF6", activityLabel: "passive")
        ]
    }

    private func check() throws { if offline { throw APIError.offline } }
    func paths() async throws -> [Path] { try check(); return empty ? [] : values }
    ...
    func create(name: String, description: String?, color: String) async throws -> Path {
        try check(); let path = Path(id: UUID(), name: name, description: description, status: "ACTIVE", color: color, activityLabel: "passive"); values.insert(path, at: 0); return path
    }
    func update(id: UUID, name: String, description: String?, color: String) async throws -> Path {
        try check(); let old = values.first { $0.id == id }!; let path = Path(id: id, name: name, description: description, status: old.status, color: color, activityLabel: old.activityLabel); values = values.map { $0.id == id ? path : $0 }; return path
    }
    func remove(id: UUID) async throws { try check(); values.removeAll { $0.id == id } }
    func restore(id: UUID) async throws { try check() }
    func merge(source: UUID, target: UUID) async throws { try check(); values.removeAll { $0.id == source } }
    func session(id: UUID) async throws -> TrackedSession { try check(); return TrackedSession(id: id, pathId: Self.pathID, labelIds: [], startedAt: SessionFormatting.iso(Date().addingTimeInterval(-3600)), endedAt: SessionFormatting.iso(Date()), durationSeconds: 3600, description: "Replication and consistency models", source: "IOS", running: false) }
}
