...
enum CalendarPortion: CaseIterable, Identifiable, Equatable {
    ...
    var title: String { switch self { case .marker: "Marker only"; case .quarter: "¼ day"; case .half: "½ day"; case .threeQuarter: "¾ day"; case .full: "Full day" } }
...
struct CalendarAPI: CalendarTransport {
    ...
    let token: String
    func labels() async throws -> [KBLabel] { try await client.request("/labels?scope=CALENDAR", token: token) }
    func days(startDate: String, endDate: String) async throws -> [CalendarDay] { try await client.request("/calendar/days?startDate=\(startDate)&endDate=\(endDate)", token: token) }
    func saveDay(date: String, request: CalendarDayRequest) async throws -> CalendarDay { try await client.request("/calendar/days/\(date)", method: "PUT", body: JSONEncoder().encode(request), token: token) }
    func saveRange(request: CalendarRangeRequest) async throws -> [CalendarDay] { try await client.request("/calendar/days/range", method: "PUT", body: JSONEncoder().encode(request), token: token) }
    func createLabel(name: String, color: String) async throws -> KBLabel {
        let body = try JSONSerialization.data(withJSONObject: ["name": name, "color": color, "scopes": ["CALENDAR"]])
