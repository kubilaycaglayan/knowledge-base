...
@MainActor @Observable final class NotesModel {
  ...
  func load(force: Bool = false) async {
    ...
    error = nil
    let key = cacheKey
    if !force, let cached = pages[key] {
      apply(cached)
      loading = false
      return
    }
    do {
      let result = try await transport.page(
        page: page, size: size, query: query, archived: archived)
      pages[key] = result
      apply(result)
  ...
  func loadLabels() async {
    do { labels = try await transport.labels() } catch {
  ...
  func open(_ note: Note) async {
    error = nil
    do { selected = try await transport.fetch(id: note.id) } catch {
      fail(error, "Unable to open this note. Please try again.")
    }
  }
  ...
  func create() async -> Note? {
    ...
    defer { busy = false }
    do {
      let note = try await transport.create(NoteDraft())
      pages.removeAll()
      return note
    } catch {
      fail(error, "Unable to create a note. Please try again.")
      return nil
    }
  }
  ...
  func save(_ draft: NoteDraft) async -> Bool {
    ...
    do {
      let saved: Note
      do {
        saved = try await transport.update(id: original.id, draft: draft, version: original.version)
  ...
  func archive(_ note: Note) async -> Bool {
    await mutate { try await self.transport.archive(id: note.id) }
  }
  ...
  private var cacheKey: String {
    "\(page):\(size):\(archived):\(query.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())"
  }
  private func apply(_ value: NotePage) {
    notes = value.items
    page = value.page
  ...
  private func mutate(_ request: () async throws -> Void) async -> Bool {
    ...
    do {
      ...
      fail(error, "Unable to update this note. Please try again.")
      return false
    }
  }
