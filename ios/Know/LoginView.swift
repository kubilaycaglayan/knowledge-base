...
struct LoginView: View {
  ...
  private var accent: Color {
    dark ? Color(red: 0.77, green: 0.82, blue: 0.89) : Color(red: 0.20, green: 0.25, blue: 0.33)
  }
  var body: some View {
    GeometryReader { geometry in
      ScrollView {
        VStack(alignment: .leading, spacing: 28) {
          ...
          VStack(alignment: .leading, spacing: 24) {
            ...
            if register {
              VStack(alignment: .leading, spacing: 8) {
                ...
                ZStack(alignment: .trailing) {
                  ...
                  .accessibilityHint(
                    passwordConfirmationVisible
                      ? "Password confirmation is visible." : "Password confirmation is hidden."
                  )
            ...
            Button(action: submit) {
              HStack {
                if model.isAuthenticating {
                  ProgressView().tint(dark ? .black : .white).accessibilityHidden(true)
                }
            ...
            if GoogleAuthentication.isConfigured {
              HStack {
                ...
                Text("or continue with").font(.caption).fixedSize()
                Rectangle().frame(height: 1)
          ...
          .background(
            dark ? Color(red: 0.11, green: 0.14, blue: 0.19) : .white,
  ...
  private func validation(_ message: String) -> some View {
    Label(message, systemImage: "exclamationmark.circle").font(.footnote)
      .foregroundStyle(
        dark ? Color(red: 1, green: 0.67, blue: 0.67) : Color(red: 0.63, green: 0.15, blue: 0.15)
      )
  ...
  private func submit() {
    ...
    passwordError =
      (9...200).contains(password.count) ? nil : "Use a password of 9 to 200 characters."
    passwordConfirmationError =
      register && passwordConfirmation != password ? "Passwords do not match." : nil
    if emailError != nil {
      focus = .email
      return
    }
  ...
  private static func isValidEmail(_ value: String) -> Bool {
    ...
    guard !local.isEmpty, !domain.isEmpty,
      ...
      !domain.hasSuffix("."), !domain.contains("..")
    else { return false }
    return true
  }
}
    ...
    content.font(.body).textFieldStyle(.plain).padding(12).frame(minHeight: 48)
      .overlay(
        RoundedRectangle(cornerRadius: 4).stroke(
          focused ? Color.accentColor : .secondary, lineWidth: focused ? 2 : 1))
  }
}
