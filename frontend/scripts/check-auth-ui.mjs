...
try {
  for (const theme of ['light', 'dark']) {
    ...
    await context.route('**/api/**', route => {
      submissions++;
      return route.fulfill({ status: 401, contentType: 'application/json', body: '{"message":"Invalid credentials"}' });
    });
    ...
    for (const width of [320, 390, 430, 1440, 2560]) {
      ...
      for (const registration of [false, true]) {
        if (registration) await page.getByRole('button', { name: 'New here? Create an account', exact: true }).click();
        ...
        assert.deepEqual(audit.violations.map(v => v.id), [], label);
        await page.screenshot({ path: join(out, `${label}.png`), fullPage: true });
        results.push(label);
        if (registration) await page.getByRole('button', { name: 'Already have an account? Sign in', exact: true }).click();
      }
    }
    ...
    assert.equal(await page.getByLabel('Password', { exact: true }).inputValue(), 'fixture-password');
    await page.screenshot({ path: join(out, `${theme}-rejected-registration.png`), fullPage: true });
    // Browser text enlargement approximates large text; native Dynamic Type is a separate gate.
    await page.addStyleTag({ content: 'h1 { font-size: 52px } p, label, input, button { font-size: 26px !important }' });
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
    await page.screenshot({ path: join(out, `${theme}-large-text.png`), fullPage: true });
    await context.close();
  }
  console.log(`Passed ${results.length} themed layout/accessibility audits and form behavior checks.`);
} finally {
