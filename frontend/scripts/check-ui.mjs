...
const routes = process.env.UI_ROUTES?.split(",") || [
  "/",
  "/sessions",
  "/paths",
  "/timeline",
...
function fixtures(state) {
  const long =
    state === "long"
      ? " ExtendedUnbrokenName".repeat(18).replaceAll(" ", "")
      : "";
  ...
  const paths = empty
    ...
    : [
        {
          id: "path-a",
          name: "Distributed systems" + long,
          description:
            "Replication, consistency, and resilient services." + long,
          status: "ACTIVE",
          color: "#2188FF",
        },
        {
          id: "path-b",
          name: "Reading",
          description: "Books and chapter notes.",
          status: "ACTIVE",
          color: "#2E9D68",
        },
      ];
  const note = {
          ...
          content: [
            {
              type: "text",
              text:
                "Compare leader-based replication with leaderless designs." +
                long,
            },
          ],
        },
    ...
    contentText:
      "Compare leader-based replication with leaderless designs." + long,
    tags: ["systems" + long],
    createdAt: timestamp,
    updatedAt: timestamp,
    version: 1,
    ...
    : [
        {
          ...
          labelIds: ["label-a"],
          labels,
          description: "Replication and consistency models" + long,
          source: "WEB",
          startedAt: timestamp,
  ...
  const categories = paths.map((path, index) => ({
    id: path.id,
    label: path.name,
    seconds: 3600 - index * 1800,
  }));
  const days = [
    {
      date,
      totalSeconds: empty ? 0 : 5400,
      paths: categories,
      sessionLabels: [],
      calendarNote: empty ? "" : "Finished the chapter." + long,
      calendarLabels: labels.map((label) => ({
        ...label,
        label: label.name,
        portion: 1,
      })),
  ...
  if (state === "dense") {
    for (let index = 1; index < 60; index++) {
      ...
      sessions.push({
        ...sessions[0],
        id: `entry-${index}`,
        description: `Reading session ${index}`,
      });
    }
  }
  return {
...
async function check(page, label, screenshot = true) {
  ...
  const errors = await new AxeBuilder({ page })
    ...
    .analyze();
  const violations = errors.violations.map((v) => ({
    id: v.id,
    nodes: v.nodes.map((n) => ({
      target: n.target,
...
try {
  for (const mode of ["light", "dark"]) {
    for (const state of process.env.UI_STATES?.split(",") || [
      "populated",
      "empty",
      "long",
      "dense",
      ...
      await context.route("**/api/**", async (route) => {
        ...
        if (state === "error" && !path.startsWith("/auth"))
        ...
        if (path === "/notes")
          data = url.search
            ? {
                ...
                page: 0,
                size: 20,
                totalItems: f.notes.length,
                totalPages: 1,
              }
            : f.notes;
        if (path === "/notes/note-a") data = f.note;
        if (path === "/notes/labels")
          data = [{ id: "label-a", name: "systems" }];
        ...
        if (path === "/statistics")
          data = {
            todaySeconds: state === "empty" ? 0 : 5400,
            weekSeconds: state === "empty" ? 0 : 29700,
            monthSeconds: state === "empty" ? 0 : 116100,
            weekByPath: state === "empty" ? {} : { "path-a": 18900 },
              ...
              : [
                  {
                    ...
                    detail: "Read and annotate the replication chapter.",
                    occurredAt: timestamp,
                  },
                ];
        if (path === "/labels") data = f.labels;
        if (path === "/calendar/labels") data = f.labels;
        if (path === "/calendar/days")
          data = [
            {
              date,
              note: state === "empty" ? "" : "Finished the chapter.",
              labels: f.assignments,
            },
          ];
        if (path === "/reports")
          data = {
            ...
            paths: f.categories,
            sessionLabels: [],
            calendarLabels: f.labels.map((label) => ({
              ...label,
              label: label.name,
              days: 1,
              markers: 0,
        ...
        if (path === "/paths/path-a/summary")
          data = { path: f.paths[0], trackedSeconds: 5400, recentActivity: [] };
      ...
      page.on("pageerror", (error) =>
        failures.push({ mode, state, error: error.message }),
      );
      for (const path of routes) {
        ...
        const widths =
          state === "populated" || state === "long"
            ? [1440, 1024, 390, 320, 2560]
            : [390];
        for (const width of widths) {
          await page.setViewportSize({
            width,
            height: width < 700 ? 844 : 1050,
          });
          await page.waitForFunction(() =>
            Array.from(document.querySelectorAll(".echarts")).every(
              (chart) =>
                !chart.querySelector("svg") ||
                Math.abs(
                  chart.querySelector("svg").getBoundingClientRect().width -
          ...
          if (geometry) {
            ...
            if (
              ...
              Object.keys(geometry).some(
                (key) => Math.abs(geometry[key] - previous[key]) > 1,
              )
            ) {
              failures.push({
                label: `${mode}-${state}-${width}-shell-alignment`,
                previous,
                geometry,
              });
            }
  ...
  console.log(
    `UI evidence: ${out}; ${results.length} audits, ${failures.length} failures`,
  );
  await browser.close();
  await server.close();
}
