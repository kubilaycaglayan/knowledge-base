...
export const useNotesStore = defineStore("notes", {
  state: () => ({ notes: [] as Note[], selected: null as Note | null, existingLabels: [] as NoteLabel[], pages: {} as Record<string, { items: Note[]; page: number; size: number; totalItems: number; totalPages: number }> }),
  actions: {
    setPage(key: string | Note[], page?: { items: Note[]; page: number; size: number; totalItems: number; totalPages: number }) {
    ...
    remove(id: string) {
      ...
      this.pages = Object.fromEntries(Object.entries(this.pages).map(([key, page]) => [key, { ...page, items: page.items.filter((note) => note.id !== id) }]));
    },
