...
const pathsStore = usePathsStore(), labelsStore = useLabelsStore();
const reportsStore = useReportsStore(), sessionsStore = useSessionsStore();
const current = ref<Timer | null>(null), timer = current, version = ref(0), historyVersion = ref(0);
const isRunning = computed(() => Boolean(current.value));
const pathId = ref(""), description = ref(""), selectedLabelIds = ref<string[]>([]), timerStartedAt = ref("");
const newLabel = ref(""), labelsOpen = ref(false), recentPathIds = ref<string[]>([]);
const now = ref(Date.now()), busy = ref(false), error = ref(""), socketConnected = ref(false);
let ticker: number | undefined, syncTicker: number | undefined, reconnectTicker: number | undefined;
let syncInFlight = false, timerStateVersion = 0, socket: WebSocket | undefined, saveQueued = false;
let formTimerId = "", consumers = 0;
...
function fieldFocused(field: "path" | "labels" | "description") {
  ...
  if (field === "path") return active.id === "tt-path" || Boolean(active.closest(".tracker-path-select"));
  return Boolean(active.closest("#tt-labels"));
}

function refreshTargets(value: Draft) {
  if (value.pathId && !pathsStore.paths.some(path => path.id === value.pathId)) void pathsStore.load(true).catch(() => {});
  if (value.labelIds?.some(id => !labelsStore.labels.some(label => label.id === id))) void labelsStore.loadScope("TIME_ENTRY", true).catch(() => {});
}
function applyDraft(value: Draft | null, baseline = draftBaseline) {
  const next = value || {};
  const resolved: Draft = {};
  if ((!draftHydrated && !pathId.value) || (!fieldFocused("path") && pathId.value === (baseline.pathId || ""))) { pathId.value = next.pathId || ""; resolved.pathId = next.pathId; }
  else resolved.pathId = baseline.pathId;
  if ((!draftHydrated && !description.value) || (!fieldFocused("description") && description.value === (baseline.description || ""))) { description.value = next.description || ""; resolved.description = next.description; }
  else resolved.description = baseline.description;
  if ((!draftHydrated && !selectedLabelIds.value.length) || (!fieldFocused("labels") && JSON.stringify(selectedLabelIds.value) === JSON.stringify(baseline.labelIds || []))) { selectedLabelIds.value = [...(next.labelIds || [])]; resolved.labelIds = next.labelIds; }
...
async function syncDraft() {
  ...
  if (revision === timerStateVersion && !current.value && !busy.value) applyDraft(draft);
}
function applyTimer(value: Timer | null, notifyHistory = false, submitted?: ReturnType<typeof formState>) {
  ...
  if (changed) { reportsStore.clear(); sessionsStore.clearPages(); }
  ...
  if (!value) {
    if (!previous) return;
    formTimerId = "";
    draftBaseline = { pathId: previous.pathId, labelIds: previous.labelIds || [], description: previous.description || "" };
    timerStartedAt.value = "";
    if (notifyHistory && changed) historyVersion.value++;
    return;
  }
  const newLocalForm = formTimerId !== value.id && !submitted;
  if (!fieldFocused("path") && (newLocalForm || !preserveDraft || draft.pathId === baseline.pathId)) pathId.value = value.pathId || "";
  if (!fieldFocused("labels") && (newLocalForm || !preserveDraft || JSON.stringify(draft.labelIds) === JSON.stringify(baseline.labelIds))) selectedLabelIds.value = [...(value.labelIds || [])];
  if (!fieldFocused("description") && (newLocalForm || !preserveDraft || draft.description === baseline.description)) description.value = value.description || "";
  ...
  draftBaseline = { pathId: value.pathId, labelIds: value.labelIds || [], description: value.description || "" };
...
async function load() {
  ...
  try {
    ...
    if (versionAtRequest === timerStateVersion) { applyTimer(current); if (!current) await syncDraft(); }
...
async function toggleRun() {
  ...
  try {
    if (timer.value) {
      const versionAtRequest = ++timerStateVersion;
      const stopped = await api<Timer>(`/timers/${timer.value.id}/stop`, { method: "POST", body: "{}" });
      reportsStore.clear();
      sessionsStore.clearPages();
      if (versionAtRequest === timerStateVersion) {
        applyTimer(null);
        if (stopped) applyDraft({ pathId: stopped.pathId, labelIds: stopped.labelIds, description: stopped.description }, submitted);
      }
    } else {
      const versionAtRequest = ++timerStateVersion;
      const submitted = formState();
      ...
      if (versionAtRequest === timerStateVersion) applyTimer(started, false, submitted);
      rememberPath(pathId.value);
    }
    historyVersion.value++;
  } catch { error.value = "Could not update the timer. Only one timer can run at a time."; }
  finally { pendingSubmission = undefined; busy.value = false; if (saveQueued) { saveQueued = false; void updateTimer(); } }
}
async function updateTimer(alreadyBusy = false) {
  ...
  try {
    ...
    const startedAt = target ? (timerStartedAt.value === localStartedAt(target.startedAt) ? target.startedAt : new Date(timerStartedAt.value).toISOString()) : undefined;
    const updated = await api<Timer & Draft>(target ? `/timers/${target.id}` : "/timers/draft", { method: "PUT", body: JSON.stringify({ pathId: pathId.value || null, labelIds: selectedLabelIds.value, startedAt, description: description.value.trim() || null }) });
    if (versionAtRequest === timerStateVersion) {
      if (target) applyTimer(updated, false, submitted);
      else applyDraft(updated || submitted, submitted);
    }
    if (target && updated.running === false) applyDraft({ pathId: updated.pathId, labelIds: updated.labelIds, description: updated.description }, submitted);
    rememberPath(pathId.value);
    historyVersion.value++;
...
async function sync() {
  ...
  try {
    ...
    if (!busy.value && versionAtRequest === timerStateVersion) { applyTimer(current, true); if (!current) await syncDraft(); }
...
function setCurrent(value: Timer | null) { timerStateVersion++; applyTimer(value); }
function clear() {
  timerStateVersion++;
  current.value = null;
  pathId.value = ""; description.value = ""; selectedLabelIds.value = []; timerStartedAt.value = "";
  draftBaseline = {}; draftHydrated = false; formTimerId = ""; newLabel.value = ""; error.value = "";
  pendingSubmission = undefined; saveQueued = false; recentPathIds.value = [];
  pathsStore.reset(); labelsStore.reset(); reportsStore.clear(); sessionsStore.clearPages();
  if (socket) { socket.onclose = null; socket.close(); socket = undefined; }
  version.value++;
  if (consumers > 0) queueMicrotask(() => { void load(); connectWebSocket(); });
}
return { current, version, historyVersion, isRunning, pathId, description, selectedLabelIds, timerStartedAt,
  newLabel, labelsOpen, recentPathIds, now, busy, error, acquire, release, setCurrent, clear,
  toggleRun, updateTimer, toggleLabel, createLabel, rememberPath, sync };
});
