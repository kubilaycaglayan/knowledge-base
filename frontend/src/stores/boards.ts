...
export const useBoardsStore = defineStore("boards", {
  ...
  actions: {
    ...
    async createBoard(name: string) { if (this.creatingBoard) return null; this.creatingBoard = true; try { const board = await api<Board>("/boards", { method: "POST", body: JSON.stringify({ name }) }); this.boards.push(board); this.forgetAllView(); this.selectedId = board.id; await this.loadBoard(); return board; } finally { this.creatingBoard = false; } },
