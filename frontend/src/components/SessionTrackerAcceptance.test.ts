...
describe("session tracker acceptance", () => {
  ...
  const labels = ["Focus", "Review", "Planning"].map((name, index) => ({ id: `label-${index}`, name, scopes: ["TIME_ENTRY"] }));
  beforeEach(() => {
    setActivePinia(createPinia());
    vi.mocked(api).mockReset();
    vi.mocked(api).mockImplementation(async (path, options = {}) => {
      if (path === "/paths") return [{ id: "active", name: "Study", status: "ACTIVE" }, { id: "archived", name: "Archived", status: "ARCHIVED" }];
  ...
  async function render() {
    wrapper = mount(FloatingTimeTracker, { attachTo: document.body, props: { inline: true }, global: { plugins: [vuetify] } });
    await flushPromises();
  }
  const chips = () => wrapper.findAll("#tt-label-options button");
  const chip = (name: string) => chips().find(button => button.text().replace("×", "") === name)!;
  const selected = () => chips().filter(button => button.attributes("aria-pressed") === "true").map(button => button.text().replace("×", ""));
  ...
  it("L3–L4, L6–L10, L14–L15, L19–L20: preserves the selected set and prioritizes it when closed", async () => {
    ...
    expect(chips().every(button => button.attributes("aria-pressed") === "false")).toBe(true);
    ...
    expect(chips().map(button => button.text())).toEqual(["Review×", "Planning×", "Focus"]);
    await chip("Review").trigger("click");
  ...
  it("L11–L12, L18: keeps internal focus open and dismisses on Escape, outside pointer, or focus leaving", async () => {
    ...
    await toggle().trigger("click");
    await wrapper.get(".label-picker").trigger("focusout", { relatedTarget: chip("Review").element });
    expect(toggle().attributes("aria-expanded")).toBe("true");
    await wrapper.get(".label-picker").trigger("focusout", { relatedTarget: wrapper.get("textarea").element });
    expect(toggle().attributes("aria-expanded")).toBe("false");
    await toggle().trigger("click");
  ...
  it.each([false, true])("L16–L17: creates a label while open=%s and preserves other selections", async open => {
    await render();
    ...
    await flushPromises();
    expect(api).toHaveBeenCalledWith("/labels", expect.objectContaining({ method: "POST", body: JSON.stringify({ name: "New label", scopes: ["TIME_ENTRY"], color: null }) }));
  ...
  it("C4: renders an empty label set with a usable creation flow", async () => {
    ...
    await wrapper.get('input[aria-label="New session label name"]').setValue("First label");
