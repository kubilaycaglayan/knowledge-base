...
const candidates = computed(() => {
  ...
  return props.paths.filter((path) =>
    ...
    (!needle || `${path.name} ${path.description || ""}`.toLocaleLowerCase().includes(needle)));
});
...
watch(visible, (isVisible) => {
  ...
  query.value = "";
  targetId.value = "";
  void nextTick(() => document.querySelector<HTMLElement>("#merge-path-search")?.focus());
});
...
<template>
  <div v-if="visible" class="prompt-dialog-backdrop" @click.self="emit('cancel')">
    ...
    >
      ...
      <p class="muted">Move every session to the target path, then remove this path.</p>
      <label for="merge-path-search">Find a target path</label>
      <input id="merge-path-search" v-model="query" type="search" name="merge-path-search" autocomplete="off" placeholder="Search paths…" />
      <fieldset class="merge-path-options">
        <legend>Target path</legend>
        <label v-for="path in candidates" :key="path.id" class="merge-path-option">
          <input v-model="targetId" type="radio" name="merge-target-path" :value="path.id" />
          <span><strong>{{ path.name }}</strong><small v-if="path.description">{{ path.description }}</small></span>
        </label>
        <p v-if="!candidates.length" class="muted">No matching paths. Change your search or create another path first.</p>
      </fieldset>
      <div class="prompt-dialog-actions">
        <button type="button" class="text-button" @click="emit('cancel')">Cancel</button>
        <button type="button" class="primary" :disabled="!targetId" @click="confirm">OK</button>
      </div>
    </section>
  </div>
</template>
