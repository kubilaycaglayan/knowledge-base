...
vi.mock("@vuepic/vue-datepicker", () => ({
  VueDatePicker: {
    name: "VueDatePicker",
    props: ["modelValue", "presetDates", "multiCalendars", "formats"],
...
describe("ReportDateRange", () => {
  ...
  it("ignores incomplete or unchanged date-picker values", async () => {
    ...
    await provider.vm.$emit("update:modelValue", [new Date(2026, 7, 24), new Date(2026, 7, 30)]);
  ...
  it("syncs externally changed ranges and formats partial picker values safely", async () => {
    const wrapper = mount(ReportDateRange, { props: { modelValue: { startDate: "2026-08-24", endDate: "2026-08-30" } } });
    const provider = wrapper.findComponent({ name: "VueDatePicker" });
    const formats = provider.props("formats") as { input(value: Date[]): string };
    expect(formats.input([new Date(2026, 7, 24)])).toBe("Select a range");
    await wrapper.setProps({ modelValue: { startDate: "2026-09-01", endDate: "2026-09-07" } });
    expect((provider.props("modelValue") as Date[]).map((date) => date.toISOString().slice(0, 10))).toEqual(["2026-09-01", "2026-09-07"]);
  });
});
