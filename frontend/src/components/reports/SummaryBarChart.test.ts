...
describe("SummaryBarChart", () => {
  const days = [
    ...
    },
    {
      ...
      totalSeconds: 3600,
      paths: [{ id: "path-1", label: "Wander", seconds: 3600 }],
      calendarNote: "Annual leave",
      calendarLabels: [
        { id: "label-1", label: "Vacation", color: "#009688", portion: 1 },
      ],
    },
  ];
  ...
  it("adds calendar inputs as label-colored shadow bars without changing time tracks", () => {
    ...
    const series = (
      wrapper.getComponent({ name: "VChart" }).props("option") as {
        series: Array<{
          name: string;
          type: string;
          yAxisIndex?: number;
    ...
    expect(series[0]).toMatchObject({
      ...
      type: "custom",
      yAxisIndex: 1,
      data: [{ value: [1, 0, 1, "#009688"] }],
    });
    expect(series[1]).toMatchObject({ name: "Wander" });
    expect(
      (
        wrapper.getComponent({ name: "VChart" }).props("option") as {
          tooltip: { confine: boolean; extraCssText: string };
        }
      ).tooltip,
    ).toMatchObject({
      confine: true,
      extraCssText: expect.stringContaining("max-width: 320px"),
    });
    const pattern = series[0].renderItem?.(
      {},
      {
        value: (index) => [0, 0, 1, "#009688"][index],
        coord: ([, value]) => [100, 200 - value * 100],
        size: () => [20, 0],
      },
    );
    ...
    expect(
      pattern?.children
        .slice(1)
        .some((child) => child.style.fill === "#ffffff"),
    ).toBe(true);
    const tooltip = (
      wrapper.getComponent({ name: "VChart" }).props("option") as {
        tooltip: { formatter(params: unknown): string };
      }
    ).tooltip.formatter([{ axisValue: "Fri, Sep 4", dataIndex: 0 }]);
    expect(tooltip).toContain("Fri, Sep 4");
    expect(tooltip).toContain("Annual leave");
    expect(tooltip).toContain(
      '<div class="tooltip-heading"><strong>Fri, Sep 4</strong><span>Total: 01:00:00</span></div>',
    );
  });
  ...
  it("uses each path color for its bars and tooltip marker", () => {
    const wrapper = mount(SummaryBarChart, {
      props: {
        ...
        categories: [
          { id: "path-1", label: "Wander", seconds: 3600, color: "#123456" },
        ],
      },
    });
    ...
    expect(
      option.tooltip.formatter([{ axisValue: "Fri, Sep 4", dataIndex: 0 }]),
    ).toContain("background:#123456");
  });

  it("omits calendar inputs when disabled", () => {
    const wrapper = mount(SummaryBarChart, {
      props: { days, categories, showCalendar: false },
    });
    const series = (
      wrapper.getComponent({ name: "VChart" }).props("option") as {
  ...
  it("adds compact totals to each x-axis label without repeating a separate row", () => {
    ...
    expect(axisLabel.formatter("Aug 2026", 0)).toBe(
      "{bucket|Aug 2026}\n{total|1h}",
    );
    expect(axisLabel.formatter("Sep 2026", 1)).toBe(
      "{bucket|Sep 2026}\n{total|2h 3m}",
    );
    expect(axisLabel.rich).toMatchObject({
      bucket: { fontSize: 11 },
  ...
  it("escapes calendar tooltip content and enables zoom for long ranges", () => {
    ...
    const wrapper = mount(SummaryBarChart, {
      props: { days: longDays, categories: [], showCalendar: true },
    });
    const option = wrapper.getComponent({ name: "VChart" }).props("option") as {
      dataZoom: unknown[];
    ...
    const tooltip = option.tooltip.formatter([
      { axisValue: "Sat, Aug 1", dataIndex: 0 },
    ]);
    expect(tooltip).toContain(
      "&lt;script&gt;alert(&#39;&amp;&#39;)&lt;/script&gt;",
    );
  ...
  it("renders marker-only and zero-portion calendar labels with safe fallback colors", () => {
    ...
    const tooltip = option.tooltip.formatter([
      { axisValue: "Sun, Sep 6", dataIndex: 0 },
    ]);
