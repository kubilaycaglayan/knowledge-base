  ...
  series: [
    {
      ...
      label: { show: false },
      data: props.categories.map((item) => ({
        name: item.label,
        value: item.seconds,
...
<template>
  <div
    class="donut-wrap"
    role="img"
    :aria-label="`Time by category: ${categories.map((item) => `${item.label}: ${formatDurationHoursMinutes(item.seconds)}`).join('; ') || 'No tracked time'}`"
  >
    <v-chart
      class="donut-echart"
      :option="option"
    ...
    <div class="donut-total">
      <strong>{{ formatDurationHoursMinutes(totalSeconds) }}</strong
      ><span>Total</span>
    </div>
  </div>
</template>
