...
export const vDialogFocus: ObjectDirective<HTMLElement> = {
  mounted(element) {
    ...
    const controls = () => Array.from(element.querySelectorAll<HTMLElement>(focusable)).filter(control => control.getClientRects().length);
    ...
    queueMicrotask(() => { if (element.isConnected) (controls()[0] || element).focus(); });
    cleanup.set(element, () => {
