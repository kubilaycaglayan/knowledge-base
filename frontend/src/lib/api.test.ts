...
describe("api", () => {
  ...
  it("sends JSON headers and the stored bearer token, then parses JSON", async () => {
    ...
    await expect(
      api<{ ok: boolean }>("/paths", { method: "POST", body: "{}" }),
  ...
  it("turns server failures into a human-readable error and preserves technical details", async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue({ status: 500, ok: false, text: async () => body }),
    );
  ...
  it("uses the API error envelope for actionable client errors", async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ...
        text: async () =>
          JSON.stringify({
            error: "description: size must be between 0 and 5000",
          }),
      }),
    );
    await expect(api("/timers")).rejects.toThrow(
  ...
  it.each([502, 503, 504])("preserves sign-in on HTTP %s", async (status) => {
    ...
    await expect(api("/paths")).rejects.toThrow(
      "Something went wrong. Please try again.",
    );
    expect(localStorage.getItem("know_token")).toBe("valid-token");
  });
