...
describe("application theme", () => {
  beforeEach(() => { localStorage.clear(); applyTheme("light"); });
  ...
  ])("bootstraps saved preference %s with system dark %s", (saved, dark, expected) => {
  ...
  it("boots when storage is blocked", () => {
    const root = { dataset: {}, style: {} };
    runBootstrap({
      document: { documentElement: root }, localStorage: { getItem: () => { throw new Error("blocked"); } },
