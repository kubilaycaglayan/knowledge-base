...
describe("SessionsView", () => {
  ...
  it("places the session tracker inline at the top of the sessions page", async () => {
    ...
    expect(wrapper.element.firstElementChild?.getAttribute("data-test")).toBe(
      "session-tracker",
    );
  });
  ...
  it("reuses the cached session page when the view is mounted again", async () => {
    ...
    await flushPromises();
    const initialHistoryCalls = vi
      .mocked(api)
      .mock.calls.filter(([path]) => path.startsWith("/time-entries?")).length;
    first.unmount();
    ...
    expect(
      vi
        .mocked(api)
        .mock.calls.filter(([path]) => path.startsWith("/time-entries?"))
        .length,
    ).toBe(initialHistoryCalls);
  ...
  it("lists sessions by latest completion time with path and label context", async () => {
    ...
    expect(
      latestSession.get(".session-title-chip").attributes("style"),
    ).toContain("--session-path-color: #2878D5");
  });
  ...
  it("groups sessions by relative dates before falling back to month and year", async () => {
    ...
    try {
      ...
      expect(
        wrapper
          .findAll(".session-group-heading")
          .map((heading) => heading.findAll("span")[0].text()),
      ).toEqual([
        "Today",
        "Yesterday",
        "This week",
      ...
      expect(
        wrapper
          .findAll(".session-group-duration")
          .map((duration) => duration.text()),
      ).toEqual(["01:01", "00:30", "00:00", "00:00", "00:00", "00:00"]);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("renders multiple session labels and marks deleted references clearly", async () => {
    vi.mocked(api).mockImplementation(async (path: string) => {
          ...
          sessions: [
            {
              ...
              durationSeconds: 3600,
              source: "WEB",
              labelIds: ["label-1", "removed-label"],
            },
          ],
        };
      ...
      if (path === "/labels?scope=TIME_ENTRY")
        return [
          { id: "label-1", name: "Vue", color: null, scopes: ["TIME_ENTRY"] },
        ];
      return undefined;
    });
    const wrapper = mount(SessionsView);
    ...
    expect(wrapper.get(".session-card-labels").text()).toContain(
      "Removed label",
    );
  ...
  it("rejects an edit when either time field is missing", async () => {
    ...
    expect(wrapper.get('[role="alert"]').text()).toBe(
      "A session needs both a start and an end time.",
    );
  ...
  it("reports update and removal failures", async () => {
    vi.mocked(api).mockImplementation(
      async (path: string, options: RequestInit = {}) => {
        if (path.startsWith("/time-entries?") && !options.method)
          return {
            page: 0,
            totalPages: 1,
            totalSessions: 1,
