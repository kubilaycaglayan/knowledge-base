...
const monthOptions = Array.from({ length: 12 }, (_, index) => ({ value: index, label: format(new Date(2024, index, 1), "MMMM") }));
...
function changeMonth(monthIndex: number) { month.value = new Date(month.value.getFullYear(), monthIndex, 1); selected.value = format(startOfMonth(month.value), "yyyy-MM-dd"); void load(); }
function changeYear(year: number) { month.value = new Date(year, month.value.getMonth(), 1); selected.value = format(startOfMonth(month.value), "yyyy-MM-dd"); void load(); }
...
<template>
  <section class="calendar-page">
    <header class="calendar-header"><div><p class="eyebrow">DAILY RECORDS</p><h1>Calendar</h1><p class="lede">Record leave, milestones, and the days worth remembering.</p></div><div class="calendar-navigation"><button class="ghost" aria-label="Previous month" @click="previousMonth">←</button><div class="calendar-picker" aria-label="Choose calendar month"><select aria-label="Calendar month" :value="month.getMonth()" @change="changeMonth(Number(($event.target as HTMLSelectElement).value))"><option v-for="option in monthOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><select aria-label="Calendar year" :value="month.getFullYear()" @change="changeYear(Number(($event.target as HTMLSelectElement).value))"><option v-for="year in yearOptions" :key="year" :value="year">{{ year }}</option></select></div><button class="ghost" aria-label="Next month" @click="nextMonth">→</button></div></header>
    <p v-if="error" class="notice" role="alert">{{ error }}</p>
    <div class="calendar-layout"><section class="calendar-grid" aria-label="Calendar"><span v-for="dayName in ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']" :key="dayName" class="calendar-weekday">{{ dayName }}</span><button v-for="day in gridDays" :key="day.toISOString()" class="calendar-day" :class="{ muted: day.getMonth() !== month.getMonth(), selected: format(day, 'yyyy-MM-dd') === selected, 'in-range': selectedRange && format(day, 'yyyy-MM-dd') >= selectedRange.start && format(day, 'yyyy-MM-dd') <= selectedRange.end }" :aria-pressed="format(day, 'yyyy-MM-dd') === selected" @mousedown.left.prevent="startPointerRange(day)" @mouseenter="previewPointerRange(day)" @mouseup.left="finishPointerRange(day)" @click="handleDayClick(day)"><time>{{ format(day, "d") }}</time><span class="calendar-day-details"><span class="calendar-labels"><span v-for="label in days[format(day, 'yyyy-MM-dd')]?.labels.slice(0, 2)" :key="label.labelId" class="calendar-label" :style="{ '--calendar-label-color': labelColor(label.labelId) }" :title="label.name"><i></i>{{ label.name }}</span><small v-if="(days[format(day, 'yyyy-MM-dd')]?.labels.length || 0) > 2">+{{ days[format(day, 'yyyy-MM-dd')].labels.length - 2 }}</small></span><small v-if="days[format(day, 'yyyy-MM-dd')]?.note">Note</small></span></button></section>
