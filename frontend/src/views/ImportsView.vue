...
const totalHistoryPages = computed(() =>
  Math.max(1, Math.ceil(batches.value.length / HISTORY_PAGE_SIZE)),
);
...
async function load() {
  try {
    const source =
      activeTab.value === "knowledge-base" ? "knowledge-base" : "clockify";
...
async function importKnowledgeBase() {
  ...
  try {
    if (!knowledgeBaseCsv.value.trim())
      throw new Error("Paste a Knowledge Base CSV file first.");
    const summary = await api<ImportSummary>("/imports/knowledge-base", {
      method: "POST",
      headers: { "Content-Type": "text/csv" },
      body: knowledgeBaseCsv.value,
    });
    ...
    importingKnowledgeBase.value = false;
  }
}
async function undo(batch: ImportBatch) {
  const sourceLabel =
    batch.source === "KNOWLEDGE_BASE" ? "Knowledge Base" : "Clockify";
  if (
    !confirm(`Undo ${sourceLabel} import from ${formatDate(batch.createdAt)}?`)
  )
    return;
  try {
    const result = await api<{
      ...
      `/imports/${batch.source === "KNOWLEDGE_BASE" ? "knowledge-base" : "clockify"}/batches/${batch.id}`,
      { method: "DELETE" },
    );
...
<template>
  <section>
    ...
    <div
      v-if="!props.knowledgeBaseOnly"
      class="import-tabs"
      role="tablist"
      aria-label="Import source"
    >
      <button
        id="imports-tab-knowledge-base"
        type="button"
        role="tab"
        aria-controls="imports-panel-knowledge-base"
        :aria-selected="activeTab === 'knowledge-base'"
        :class="{ selected: activeTab === 'knowledge-base' }"
        @click="selectTab('knowledge-base')"
      >
        Knowledge Base
      </button>
      <button
        id="imports-tab-clockify"
        type="button"
        role="tab"
        aria-controls="imports-panel-clockify"
        :aria-selected="activeTab === 'clockify'"
        :class="{ selected: activeTab === 'clockify' }"
        @click="selectTab('clockify')"
      >
        Clockify
      </button>
    </div>
    <section
      v-if="activeTab === 'clockify' && !props.knowledgeBaseOnly"
      id="imports-panel-clockify"
      class="card import-panel"
      role="tabpanel"
      aria-labelledby="imports-tab-clockify"
      tabindex="0"
    >
      <textarea
    ...
    <section
      v-else
      id="imports-panel-knowledge-base"
      class="card import-panel"
      role="tabpanel"
      aria-labelledby="imports-tab-knowledge-base"
      tabindex="0"
    >
      <p class="muted">
        Import a CSV previously downloaded from Settings. Path and label colors,
        scopes, and assignments are restored. Existing records and conflicting
        records are skipped.
      </p>
        ...
        <input
          ...
          @change="
            async (event) => {
              ...
              if (file) knowledgeBaseCsv = await file.text();
            }
          "
        />
      </label>
      <textarea
        v-model="knowledgeBaseCsv"
        rows="10"
        aria-label="Knowledge Base CSV"
        placeholder="Paste a Knowledge Base export CSV here…"
      ></textarea>
      <div class="row-actions">
        <button
          class="primary"
          type="button"
          :disabled="!knowledgeBaseCsv.trim() || importingKnowledgeBase"
          @click="importKnowledgeBase"
        >
          {{
            importingKnowledgeBase ? "Importing…" : "Import Knowledge Base data"
          }}
        </button>
        <span v-if="importSummary" class="muted" role="status">{{
          importSummary
    ...
    <section class="card history-box">
      ...
      >
        ...
        <span class="muted"
          >Page {{ historyPage }} of {{ totalHistoryPages }}</span
        >
        <button
          type="button"
      ...
      <p v-if="!batches.length" class="muted">
        No
        {{
          props.knowledgeBaseOnly
            ? "Knowledge Base"
            : activeTab === "clockify"
              ? "Clockify"
              : "Knowledge Base"
...
.import-tabs button {
  ...
  border-radius: calc(var(--workspace-radius) - 1px);
