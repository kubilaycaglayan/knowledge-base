      ...
      <article v-for="session in group.sessions" :key="session.id" class="card session-card">
        ...
        <form v-else-if="draft" class="session-edit" @submit.prevent="save(session)">
          ...
          <div class="session-edit-grid">
            <label class="session-edit-description">Description <span>(optional)</span><textarea v-model="draft.description" name="session-description" autocomplete="off" aria-label="Edit session description" maxlength="5000" rows="1" placeholder="What did you work on…"></textarea></label>
