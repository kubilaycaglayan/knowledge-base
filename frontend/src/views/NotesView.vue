...
<template>
  <section class="notes-page">
    <header class="notes-heading">
      ...
      <button v-if="!showArchived" class="icon-button" aria-label="Create new note" title="Create new note" @click="newNote"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg></button>
    </header>

    <p v-if="error" class="notes-error" role="alert">{{ error }}</p>
    <template v-if="!isEditor">
      <div class="notes-toolbar">
        <label class="search-field"><span class="sr-only">Search notes</span><input v-model="query" name="note-search" autocomplete="off" aria-label="Search notes" placeholder="Search title, body, or label…" /></label>
        <label class="size-field"><span>Show</span><select v-model="size" name="notes-per-page" aria-label="Notes per page"><option :value="20">20</option><option :value="50">50</option><option :value="100">100</option></select></label>
      ...
      <div v-else class="note-list" aria-label="Notes">
        <div v-for="note in notes" :key="note.id" class="note-row" :class="{ archived: showArchived }">
          <component :is="showArchived ? 'span' : 'RouterLink'" :to="{ name: 'note-editor', params: { id: note.id } }" class="note-row-main"><strong>{{ note.title }}</strong><span><em v-if="isEmptyNote(note)">Empty note</em><template v-else>{{ excerpt(note) }}</template></span></component>
    ...
    <template v-else>
      <div class="editor-toolbar" aria-label="Formatting toolbar">
        ...
        <span role="status" class="save-state" :class="status">{{ status === "saving" ? "Saving…" : status === "error" ? "Not saved" : "Saved" }}</span>
...
<style scoped>
.notes-page { max-width: 1200px; margin: 0 auto; color: var(--workspace-text); }
.notes-heading, .notes-toolbar, .editor-toolbar, .notes-pagination { display: flex; align-items: center; gap: 16px; }
.notes-heading { justify-content: space-between; margin-bottom: 24px; }
.notes-heading h1 { margin: 8px 0 0; font-size: 26px; }
.icon-button { width: 46px; height: 46px; border: 0; border-radius: var(--workspace-radius); background: var(--workspace-accent); color: var(--workspace-on-accent); font-size: 28px; line-height: 1; cursor: pointer; }
.notes-toolbar { margin-bottom: 16px; }
.search-field { flex: 1; }.search-field input, .size-field select, .tag-editor input { width: 100%; border: 1px solid var(--workspace-control-border); background: var(--workspace-surface); padding: 9px 11px; font: inherit; border-radius: var(--workspace-radius); }.size-field { display:flex; gap: 8px; align-items:center; color:var(--workspace-muted); white-space:nowrap; }.size-field select { width:auto; }
.note-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(100%, 280px), 1fr)); align-items: stretch; gap: 12px; padding-top: 8px; }
.note-row { width: 100%; min-height: 184px; display: flex; flex-direction: column; justify-content: space-between; gap: 20px; text-align: left; border: 1px solid var(--workspace-border); border-radius: 10px; padding: 18px; background: var(--workspace-surface); transition: background-color 160ms ease, border-color 160ms ease, box-shadow 160ms ease, transform 160ms ease; }
.note-row:hover, .note-row:focus-within { background: var(--workspace-hover); border-color: var(--workspace-border); box-shadow: 0 5px 16px rgb(0 0 0 / 7%); }
.note-row-main { min-width: 0; display: grid; align-content: start; gap: 8px; }
.note-row-main strong { display: -webkit-box; overflow: hidden; font-size: 16px; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
.note-row-main span { display: -webkit-box; overflow: hidden; color: var(--workspace-muted); line-height: 1.5; -webkit-box-orient: vertical; -webkit-line-clamp: 3; }
.note-row-meta { display: grid; grid-template-columns: minmax(0, 1fr) auto; justify-items: stretch; gap: 10px; color: var(--workspace-muted); font-size: 12px; white-space: nowrap; }
.note-tags { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 5px; }
.note-row-meta > .note-tags { grid-column: 1 / -1; }
.note-row-meta > time { grid-column: 1; align-self: center; justify-self: start; }
.note-row-meta > span:last-child { grid-column: 2; justify-self: end; }
.note-tags i, .note-tag { font-style: normal; color: var(--workspace-muted); background: var(--workspace-selected); padding: 4px 8px; font-size: 11px; }
.notes-pagination { justify-content: space-between; padding: 18px 0; color: var(--workspace-muted); font-size: 13px; }
.notes-pagination div { display: flex; align-items: center; gap: 10px; }
.notes-empty, .notes-error { padding: 30px 0; color: var(--workspace-muted); }
.notes-error { color: var(--workspace-danger); }
.editor-toolbar { border-bottom:1px solid var(--workspace-border); padding:0 0 12px; }.toolbar-spacer { flex:1; }.save-state { color:var(--workspace-muted); font-size:13px; }.save-state.error { color:var(--workspace-danger); }.note-title-input { width:100%; border:0;  background:transparent; font:inherit; font-size:26px; font-weight:650; letter-spacing:-1px; padding:22px 0 14px; }.tag-editor { position:relative; display:flex; flex-wrap:wrap; gap:7px; align-items:center; border-bottom:1px solid var(--workspace-border); padding:0 0 14px; }.tag-editor input { border:0; padding:6px 0; width:180px; flex:1; min-width:150px; }.note-tag button { min-width:24px; min-height:24px; border:0; background:transparent; color:inherit; cursor:pointer; padding:0 0 0 5px; }.label-suggestions { position:absolute; z-index:2; top:calc(100% - 8px); left:0; min-width:220px; max-width:min(360px, 100%); display:grid; padding:5px; border:1px solid var(--workspace-border); background:var(--workspace-surface);  }.label-suggestion { border:0; background:transparent; color:var(--workspace-muted); cursor:pointer; padding:8px 10px; text-align:left; font:inherit; }.label-suggestion:hover, .label-suggestion:focus-visible, .label-suggestion.active { background:var(--workspace-selected);  }.rich-editor { min-height:420px; padding:20px 0; }.rich-editor :deep(.ProseMirror) { min-height:380px;  line-height:1.45; }.rich-editor :deep(h1), .rich-editor :deep(h2), .rich-editor :deep(h3) { letter-spacing:-1px; }.rich-editor :deep(ul[data-type="taskList"]) { list-style:none; padding-left:0; }.rich-editor :deep(ul[data-type="taskList"] li) { display:flex; gap:8px; }.rich-editor :deep(ul[data-type="taskList"] li > label) { margin-top:5px; }.note-dates { color:var(--workspace-muted); font-size:12px; border-top:1px solid var(--workspace-border); padding-top:12px; }.sr-only { position:absolute; width:1px; height:1px; padding:0; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0; }
@media(max-width:650px) { .notes-toolbar { align-items:stretch; flex-direction:column; }.note-row-meta { display:grid; align-items:center; }.notes-pagination { align-items:flex-start; flex-direction:column; }.notes-pagination div { width:100%; justify-content:space-between; }.notes-heading { margin-bottom:22px; } }
.note-row-main { flex: 1; color: inherit; text-decoration: none; }
.note-tag { min-width: 0; max-width: 100%; overflow-wrap: anywhere; }
.icon-button { display: inline-flex; align-items: center; justify-content: center; }
.note-row-main:hover strong { text-decoration: underline; }
.note-row-meta { min-width: 0; max-width: 45%; white-space: normal; }
.note-tags i { overflow-wrap: anywhere; }
