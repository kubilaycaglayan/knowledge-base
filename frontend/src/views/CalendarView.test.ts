...
describe("CalendarView", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(api).mockImplementation(
      async (path: string, options?: RequestInit) => {
        if (path === "/labels?scope=CALENDAR" && !options)
          return [
            {
              id: "leave",
              name: "Sick leave",
  ...
  it("creates a reusable label on demand", async () => {
    ...
    expect(vi.mocked(api)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"name":"Vacation"'),
      }),
    );
  ...
  it("creates a label when a mobile keyboard emits a line-break input", async () => {
    ...
    expect(vi.mocked(api)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"name":"Mobile label"'),
      }),
    );
  });
  ...
  it("shows actionable errors when calendar mutations fail", async () => {
    ...
    await flushPromises();
    expect(wrapper.get('[role="alert"]').text()).toBe(
      "Unable to add that label. Label names must be unique.",
    );
