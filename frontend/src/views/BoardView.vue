...
<template>
  <section ref="boardPage" class="board-page" aria-labelledby="board-heading">
    ...
    <div v-if="editing" class="dialog-backdrop" role="presentation" @click.self="closeEditor()"><section v-dialog-focus class="card-editor" :class="{ accented: draftAccent }" :style="{ '--card-accent': draftAccent }" role="dialog" aria-modal="true" aria-label="Edit card" tabindex="-1" @keydown.esc.prevent="closeEditor()" @keydown.meta.enter.prevent="closeEditor()" @keydown.ctrl.enter.prevent="closeEditor()">
      <div class="card-editor-header"><textarea v-model="draft.title" class="card-title-input" name="title" aria-label="Title" maxlength="240" rows="1" autocomplete="off" @keydown.enter.exact.prevent="cardEditor?.commands.focus('start')"></textarea><button class="icon-button quiet editor-close" type="button" aria-label="Close card" title="Close" @click="closeEditor()"><v-icon :icon="mdiClose" size="20" aria-hidden="true" /></button></div>
      <div v-if="!selectedBoard?.pathId || labelsStore.forScope('BOARD').length" class="card-meta">
