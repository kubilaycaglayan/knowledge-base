...
const editing = ref<BoardCard | null>(null), draft = ref({ title: "", body: "{}", priority: "MEDIUM" as BoardCard["priority"], startDate: "", dueDate: "", pathIds: [] as string[], labelIds: [] as string[] });
...
const addDays = (value: string, amount: number) => { const date = parseDate(value); date.setDate(date.getDate() + amount); return isoDate(date); };
const today = isoDate(new Date());
const ganttFrom = ref(typeof route.query.from === "string" ? route.query.from : today);
const ganttTo = ref(typeof route.query.to === "string" ? route.query.to : addDays(ganttFrom.value, 13));
const timelineDays = computed(() => { const days: string[] = []; const cursor = parseDate(ganttFrom.value); const end = parseDate(ganttTo.value); while (cursor <= end && days.length < 366) { days.push(isoDate(cursor)); cursor.setDate(cursor.getDate() + 1); } return days; });
const timelineWidth = computed(() => Math.max(100, timelineDays.value.length * 56));
...
const barStyle = (card: BoardCard) => { const days = timelineDays.value; const start = card.startDate || card.dueDate; const end = card.dueDate || card.startDate; if (!start || !end || !days.length) return {}; const first = Math.max(0, days.indexOf(start) < 0 ? (start < days[0] ? 0 : days.length - 1) : days.indexOf(start)); const last = Math.min(days.length - 1, days.indexOf(end) < 0 ? (end > days.at(-1)! ? days.length - 1 : 0) : days.indexOf(end)); return { left: `${(first / days.length) * 100}%`, width: `${(Math.max(1, last - first + 1) / days.length) * 100}%` }; };
...
function shiftGantt(amount: number) { ganttFrom.value = addDays(ganttFrom.value, amount); ganttTo.value = addDays(ganttTo.value, amount); updateGanttRange(); }
...
function editCard(card: BoardCard) { editing.value = card; draft.value = { title: card.title, body: card.body, priority: card.priority, startDate: card.startDate || "", dueDate: card.dueDate || "", pathIds: [...card.pathIds], labelIds: [...card.labelIds] }; }
async function saveCard() { if (!editing.value) return; try { await store.updateCard(editing.value, { ...draft.value, startDate: draft.value.startDate || undefined, dueDate: draft.value.dueDate || undefined }); editing.value = null; } catch { error.value = "Could not save card."; } }
