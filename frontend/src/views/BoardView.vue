...
function discardChanges() { discardOpen.value = false; destroyCardEditor(); editing.value = null; }
async function saveCard() { if (!editing.value) return; try { await store.updateCard(editing.value, { ...draft.value, startDate: draft.value.startDate || undefined, dueDate: draft.value.dueDate || undefined }); destroyCardEditor(); editing.value = null; discardOpen.value = false; } catch { error.value = "Could not save card."; } }
