...
async function toggleStatusSort(status: BoardStatus) { dismissError(); try { await store.setStatusSort(status, status.cardSort === "PRIORITY" ? "MANUAL" : "PRIORITY"); } catch { notices.notify("Could not sort this column."); } }
...
async function changeCardStatus(statusId: string) { const card = editing.value; if (!card || card.statusId === statusId) return; saveChain = saveChain.then(async () => { try { await store.moveCard(card, statusId, cardsFor(statusId).length); } catch { notices.notify("Could not move card. The change was rolled back."); } }); await saveChain; }
