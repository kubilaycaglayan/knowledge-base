...
const sortedByPriority = (statusId: string) => store.statuses.find((status) => status.id === statusId)?.cardSort === "PRIORITY";
const cardsFor = (statusId: string) => cards.value.filter((card) => card.statusId === statusId && !card.archived).sort(sortedByPriority(statusId) ? byPriorityThenPosition : (a, b) => a.position - b.position);
async function toggleStatusSort(status: BoardStatus) { dismissError(); try { await store.setStatusSort(status, status.cardSort === "PRIORITY" ? "MANUAL" : "PRIORITY"); } catch { error.value = "Could not sort this column."; } }
...
function dropCard(event: DragEvent, target: BoardCard) { event.stopPropagation(); const cardId = event.dataTransfer?.getData("text/plain"); const card = cards.value.find((item) => item.id === cardId); if (!card || card.id === target.id) return; if (sortedByPriority(target.statusId)) { if (card.statusId !== target.statusId) void store.moveCard(card, target.statusId, cardsFor(target.statusId).length).catch(() => { error.value = "Could not move card. The change was rolled back."; }); return; } const position = cardsFor(target.statusId).findIndex((item) => item.id === target.id); if (position < 0) return; void store.moveCard(card, target.statusId, position).catch(() => { error.value = "Could not reorder card. The change was rolled back."; }); }
...
function saveGroupOrder(group: Board[]) { store.reorderBoards(group.map((item) => item.id)).catch(() => { error.value = "Could not reorder boards."; }); }
...
<template>
  <section ref="boardPage" class="board-page" aria-labelledby="board-heading">
    ...
    <div v-if="managerOpen" class="dialog-backdrop" role="presentation" @click.self="closeManager"><section v-dialog-focus class="confirm-dialog board-settings boards-manager" role="dialog" aria-modal="true" aria-labelledby="boards-manager-title" tabindex="-1" @keydown.esc.prevent="closeManager">
    ...
    <div v-if="editing" class="dialog-backdrop" role="presentation" @click.self="closeEditor()"><section v-dialog-focus class="card-editor" :class="{ accented: draftAccent }" :style="{ '--card-accent': draftAccent }" role="dialog" aria-modal="true" aria-label="Edit card" tabindex="-1" @keydown.capture="noteLabelsMenu" @keydown.esc.prevent="escapeEditor" @keydown.meta.enter.prevent="closeEditor()" @keydown.ctrl.enter.prevent="closeEditor()">
      ...
      <div v-if="!selectedBoard?.pathId" class="card-meta">
