...
- `GET /reports?period=WEEK|MONTH|YEAR&anchor=YYYY-MM-DD` for a day-by-day report with total time and path/item categories for the selected period. Weeks use Monday through Sunday in UTC; the anchor date may be any date in the requested period. Pass both `startDate=YYYY-MM-DD` and `endDate=YYYY-MM-DD` instead to request a custom inclusive range of at most one year.
