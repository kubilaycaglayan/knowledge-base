...
- [x] **UP-03** The response also carries `recentPathIds`: up to five distinct active paths from the user's most recent time entries, newest first. It is derived from time entries, not stored separately.
  _Tests:_ `UserPreferencesIntegrationTest.recentPathsComeFromRecentTimeEntries`

## Web

- [x] **UP-04** After sign-in the app loads the preferences and applies the theme; changing the theme saves it to the server. Signed out, the theme still works from the browser cache.
  _Tests:_ `preferences.test.ts`, `theme.test.ts`
- [x] **UP-05** The Kanban full-width toggle reads and saves `kanbanWide` on the server.
