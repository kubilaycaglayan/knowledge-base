(function (root, factory) {
  ...
  function validate(payload) {
    if (
      !payload ||
      !Array.isArray(payload.timeentries) ||
      payload.timeentries.length > MAX_ENTRIES
    )
    ...
    try {
      ...
      return { ok: false, error: "Clockify report is invalid." };
    }
    if (bytes > MAX_BYTES)
      return { ok: false, error: "Clockify report is too large." };
    if (
      payload.timeentries.some(
