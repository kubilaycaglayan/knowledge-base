...
const apiOrigin = process.env.KNOW_API_BASE
  ...
  : "http://localhost:8080";
const iconVariant =
  process.env.KNOW_EXTENSION_ENV === "production"
    ? "production"
    : "development";
...
if (process.env.KNOW_EXTENSION_ENV === "production") {
  ...
  if (
    ...
    configuredApi.hostname === "localhost" ||
    configuredApi.hostname === "127.0.0.1"
  )
    throw new Error(
      "Production extension builds require a public HTTPS API host",
    );
}
...
export default defineConfig({
  ...
  manifest: {
    ...
    host_permissions: [
      `${apiOrigin}/*`,
      "https://app.clockify.me/reports/detailed*",
    ],
  ...
  vite: () => ({
    define: {
      ...
      __KNOW_EXTENSION_ENV__: JSON.stringify(
        process.env.KNOW_EXTENSION_ENV || "development",
      ),
    },
  }),
  dev: {
