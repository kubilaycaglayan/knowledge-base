(function (root, factory) {
  ...
  function parseRedirect(url, expectedState, expectedNonce) {
    ...
    if (values.get("state") !== expectedState)
      throw Error("Google sign-in state mismatch");
    const idToken = values.get("id_token");
    if (!idToken)
      throw Error(
        values.get("error") || "Google did not return an identity token",
      );
    const parts = idToken.split(".");
    if (parts.length !== 3)
      throw Error("Google returned an invalid identity token");
    const payload = JSON.parse(
      atob(parts[1].replace(/-/g, "+").replace(/_/g, "/")),
    );
    if (payload.nonce !== expectedNonce)
      throw Error("Google sign-in nonce mismatch");
    return idToken;
  }
