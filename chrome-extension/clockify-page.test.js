...
function createPage() {
  ...
  class FakeXHR {
    constructor() {
      this.listeners = {};
      this.responseText = JSON.stringify({ timeentries: [{ id: "entry-2" }] });
    }
...
const reportUrl =
  "https://app.clockify.me/report/workspaces/workspace-1/async/reports/detailed/report-1";
...
test("publishes valid detailed reports intercepted through fetch", async () => {
  ...
  assert.equal(
    JSON.stringify(
      page.messages.map(({ message, targetOrigin }) => ({
        message,
        targetOrigin,
      })),
    ),
...
test("publishes valid detailed reports intercepted through XHR", () => {
  ...
  assert.equal(
    JSON.stringify(page.messages[0].message.payload),
    JSON.stringify({ timeentries: [{ id: "entry-2" }] }),
  );
});
...
test("ignores non-report URLs", async () => {
  ...
  xhr.open(
    "GET",
    "https://example.com/report/workspaces/a/async/reports/detailed/b",
  );
  xhr.send();
  await new Promise((resolve) => setImmediate(resolve));
