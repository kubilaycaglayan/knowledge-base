...
const token = (nonce) =>
  `${encode({ alg: "none" })}.${encode({ nonce })}.signature`;
...
test("accepts only a redirect with matching state and nonce", () => {
  ...
  assert.equal(
    parseRedirect(
      `https://extension.chromiumapp.org/#state=state&id_token=${idToken}`,
      "state",
      "nonce",
    ),
    idToken,
  ...
  assert.throws(
    () =>
      parseRedirect(
        `https://extension.chromiumapp.org/#state=other&id_token=${idToken}`,
        "state",
        "nonce",
      ),
    /state mismatch/,
  ...
  assert.throws(
    () =>
      parseRedirect(
        `https://extension.chromiumapp.org/#state=state&id_token=${token("other")}`,
        "state",
        "nonce",
      ),
    /nonce mismatch/,
