...
test("all labels are available for any selected path", () => {
  const labels = [
    { id: "one", pathIds: ["path-a"] },
    { id: "two", pathIds: ["path-b"] },
  ];
  assert.deepEqual(
    timerLabels(labels, "path-a").map((label) => label.id),
    ["one", "two"],
  );
...
test("labels remain available across independent path selections", () => {
  ...
  assert.deepEqual(
    timerLabels(labels, "path-a", ["two"]).map((label) => label.id),
    ["one", "two"],
  );
});

test("timer requests carry the extension source and nullable selections", () => {
  assert.deepEqual(timerStartPayload("", ["label-id"], ""), {
    pathId: null,
    labelIds: ["label-id"],
    description: null,
...
test("current timer status does not display the server description", () => {
  ...
  assert.equal(
    timerElapsedSeconds(timer, Date.parse("2026-08-31T01:02:03Z")),
    3723,
  );
  assert.equal(
    timerStatus(timer, Date.parse("2026-08-31T01:02:03Z")),
    "01:02:03",
  );
...
test("formats session group totals as HH:MM", () => {
  assert.equal(
    formatGroupDuration([{ durationSeconds: 3600 }, { durationSeconds: 1800 }]),
    "01:30",
  );
