...
const required = [
  ...
  ['export COMPOSE_PROJECT_NAME="knowledge-base-smoke-${smoke_pid}-$(date +%s%N)"', 'generated isolated smoke Compose project name'],
  ...
  ['compose down --volumes --remove-orphans --rmi local', 'scoped Compose containers, volumes, and local images cleanup'],
  ...
  ['export BUILDX_BUILDER="$buildx_builder"', 'Buildx builds are scoped to the temporary builder'],
  ['docker-compose.smoke-gha-cache.yml', 'GitHub Actions BuildKit cache overlay'],
  ['docker-compose.smoke-local-cache.yml', 'optional local BuildKit cache overlay'],
  ['docker-compose.smoke-local-api-cache-from.yml', 'warm API BuildKit cache overlay'],
  ['docker-compose.smoke-local-web-cache-from.yml', 'warm web BuildKit cache overlay'],
