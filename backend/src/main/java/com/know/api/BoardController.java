...
public class BoardController {
  ...
  record CardRequest(String title, String body, BoardPriority priority, LocalDate startDate, LocalDate dueDate, List<UUID> pathIds, List<UUID> labelIds, Instant expectedUpdatedAt) {}
  ...
  @PutMapping("/{id}/cards/{cardId}") @Transactional public CardView updateCard(Authentication auth, @PathVariable UUID id, @PathVariable UUID cardId, @RequestBody CardRequest request) { Board b = writableBoard(auth, id); BoardCard c = card(b, cardId); if (request.expectedUpdatedAt() != null && !request.expectedUpdatedAt().equals(c.getUpdatedAt())) throw new ResponseStatusException(HttpStatus.CONFLICT, "Card changed elsewhere"); apply(auth, b, c, request); return CardView.of(cards.save(c)); }
