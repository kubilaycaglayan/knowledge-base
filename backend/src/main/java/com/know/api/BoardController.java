...
public class BoardController {
  ...
  record CardRequest(String title, String body, BoardPriority priority, LocalDate startDate, LocalDate dueDate, List<UUID> pathIds, List<UUID> labelIds) {}
  ...
  @PostMapping("/{id}/cards/{cardId}/restore") public CardView restoreCard(Authentication auth, @PathVariable UUID id, @PathVariable UUID cardId) { BoardCard c = card(writableBoard(auth, id), cardId); c.restore(); return CardView.of(cards.save(c)); }
  @GetMapping("/{id}/gantt") public List<CardView> gantt(Authentication auth, @PathVariable UUID id, @RequestParam LocalDate from, @RequestParam LocalDate to) { if (to.isBefore(from)) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "to must be on or after from"); Board b = board(auth, id); return cards.findGanttCards(b.getId(), from, to).stream().map(CardView::of).toList(); }
