...
public class BoardCard {
  ...
  public void update(String title, String body, BoardPriority priority, LocalDate start, LocalDate due) { this.title = title == null ? "" : title; this.body = body == null || body.isBlank() ? "{}" : body; this.priority = priority == null ? BoardPriority.MEDIUM : priority; this.startDate = start; this.dueDate = due; this.updatedAt = Instant.now(); }
  public void move(UUID statusId, int position) { this.statusId = statusId; this.position = position; this.updatedAt = Instant.now(); }
  public void archive() { archivedAt = Instant.now(); updatedAt = Instant.now(); } public void restore() { archivedAt = null; updatedAt = Instant.now(); }
