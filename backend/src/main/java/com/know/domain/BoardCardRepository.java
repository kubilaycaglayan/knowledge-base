...
public interface BoardCardRepository extends JpaRepository<BoardCard, UUID> {
  ...
  // Low-first columns: Low, Medium, High, Urgent, then manual position.
  @Query(value = "select * from board_cards c where c.board_id = :boardId and c.status_id = :statusId and c.archived_at is null order by case c.priority when 'LOW' then 0 when 'MEDIUM' then 1 when 'HIGH' then 2 else 3 end, c.position, c.id limit :limit offset :offset", nativeQuery = true)
