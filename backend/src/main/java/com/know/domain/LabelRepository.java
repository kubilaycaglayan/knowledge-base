...
public interface LabelRepository extends JpaRepository<Label, UUID> {
  ...
  @Query("select l from Label l where l.userId = :userId and exists (select s from LabelScope s where s.id.labelId = l.id and s.id.scope = :scope) order by l.name")
  List<Label> findAllByUserIdAndScope(@Param("userId") UUID userId, @Param("scope") LabelScopeType scope);
}
