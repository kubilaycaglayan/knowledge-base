...
public class LogService {
  ...
  public List<LogView> list(UUID userId) {
    return logs.findAllByUserIdOrderByOccurredAtDescIdDesc(userId).stream()
        .map(this::view)
        .toList();
  }
  ...
  @Transactional
  public LogView update(
      UUID userId, UUID id, String body, Instant occurredAt, Long expectedVersion) {
    Log log =
  ...
  public LogView setLabels(UUID userId, UUID id, List<UUID> requestedLabelIds) {
    ...
    Set<UUID> available =
        labels.findAllByUserIdAndScope(userId, LabelScopeType.LOG).stream()
            .map(Label::getId)
            .collect(java.util.stream.Collectors.toSet());
    if (!available.containsAll(requested))
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Log labels must be owned LOG-scoped labels");
  ...
  private LogView view(Log log) {
    return new LogView(
        ...
        log.getBody(),
        log.getOccurredAt(),
        logLabels.findAllByIdLogId(log.getId()).stream()
  ...
  private static String cleanBody(String body) {
    if (body == null || body.isBlank())
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Log text cannot be blank");
    String cleaned = body.trim();
    if (cleaned.length() > 20000)
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Log text is too long");
    return cleaned;
  }

  private static Instant requiredTimestamp(Instant timestamp) {
    if (timestamp == null)
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Log timestamp is required");
    return timestamp;
  }
}
