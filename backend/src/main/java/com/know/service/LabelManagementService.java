...
public class LabelManagementService {
  ...
  public record View(
      UUID id, String name, String color, Set<LabelScopeType> scopes, boolean system) {}
  ...
  public View create(
    ...
    Label label =
        labels
            .findByUserIdAndNameIgnoreCase(userId, normalized)
  ...
  @Transactional
  public View update(
      UUID userId, UUID id, String name, String color, Collection<LabelScopeType> requestedScopes) {
  ...
  public void delete(UUID userId, UUID id, boolean removeAssignments) {
    ...
    boolean assigned =
        calendarAssignments.existsByIdLabelId(id)
            || timeAssignments.existsByIdLabelId(id)
            || noteAssignments.existsByIdLabelId(id)
            || logAssignments.existsByIdLabelId(id);
  ...
  private void replaceScopes(Label label, Collection<LabelScopeType> requested) {
    Set<LabelScopeType> next =
        requested == null || requested.isEmpty()
            ? EnumSet.noneOf(LabelScopeType.class)
  ...
  private View view(Label label) {
    return new View(
        ...
        label.getName(),
        label.getColor(),
        scopes.findAllByIdLabelId(label.getId()).stream()
            .map(value -> value.getId().getScope())
            .collect(
  ...
  private Label owned(UUID userId, UUID id) {
    return labels
