...
public class CalendarService {
  ...
  private static final Set<String> VALID_COLORS =
      Set.of(
          "#2878D5", "#E05D44", "#D69E2E", "#2F855A", "#009688", "#805AD5", "#D53F8C", "#4A5568",
          "#718096", "#8B5E3C");
  ...
  public List<LabelView> labels(UUID userId) {
    return labels.findAllByUserIdAndScope(userId, LabelScopeType.CALENDAR).stream()
        .map(this::labelView)
        .toList();
  }
  ...
  public LabelView createLabel(UUID userId, String name, String color) {
    ...
    if (existing.isPresent()
        && scopes.existsByIdLabelIdAndIdScope(existing.get().getId(), LabelScopeType.CALENDAR))
    ...
    if (label.getColor() == null && color != null) {
      label.update(label.getName(), color);
      labels.save(label);
    }
    if (!scopes.existsByIdLabelIdAndIdScope(label.getId(), LabelScopeType.CALENDAR))
      scopes.save(new LabelScope(new LabelScopeId(label.getId(), LabelScopeType.CALENDAR)));
  ...
  public LabelView updateLabel(UUID userId, UUID id, String name, String color) {
    ...
    String normalized = normalizedName(name);
    validateColor(color);
    if (!label.getName().equalsIgnoreCase(normalized)
        && labels.existsByUserIdAndNameIgnoreCase(userId, normalized))
      conflict("A label with this name already exists");
  ...
  public void deleteLabel(UUID userId, UUID id) {
    Label label = label(userId, id);
    if (assignments.existsByIdLabelId(id)
        || timeAssignments.existsByIdLabelId(id)
        || noteAssignments.existsByIdLabelId(id)) conflict("Labels in use cannot be deleted");
    labels.delete(label);
  }

  public List<DayView> days(UUID userId, LocalDate from, LocalDate to) {
    List<DailyRecord> found =
        records.findAllByUserIdAndRecordDateBetweenOrderByRecordDate(userId, from, to);
    return views(found);
  }
  ...
  public DayView replaceDay(UUID userId, LocalDate date, String note, List<LabelInput> inputs) {
    ...
    DailyRecord record =
        existing.orElseGet(() -> records.save(new DailyRecord(userId, date, cleanedNote)));
            ...
            .map(
                input ->
                    new DailyRecordLabel(
                        new DailyRecordLabelId(record.getId(), input.labelId()), input.portion()))
            .toList());
    return view(
        record,
        requested.stream()
            .map(input -> assignmentView(owned.get(input.labelId()), input.portion()))
            .toList());
  }
  ...
  public List<DayView> applyRange(
    ...
    for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
      ...
      assignments
          .findAllByIdDailyRecordId(record.getId())
          .forEach(value -> current.put(value.getId().getLabelId(), value.getPortion()));
      ...
      assignments.saveAll(
          current.entrySet().stream()
              .map(
                  entry ->
                      new DailyRecordLabel(
                          new DailyRecordLabelId(record.getId(), entry.getKey()), entry.getValue()))
              .toList());
    }
  ...
  private List<DayView> views(List<DailyRecord> found) {
    ...
    labels
        .findAllById(
            assignments.findAllByIdDailyRecordIdIn(ids).stream()
                .map(a -> a.getId().getLabelId())
                .distinct()
                .toList())
        .forEach(label -> labelsById.put(label.getId(), label));
    Map<UUID, List<LabelAssignmentView>> byRecord = new HashMap<>();
    assignments
        .findAllByIdDailyRecordIdIn(ids)
        .forEach(
            a ->
                byRecord
                    .computeIfAbsent(a.getId().getDailyRecordId(), ignored -> new ArrayList<>())
                    .add(assignmentView(labelsById.get(a.getId().getLabelId()), a.getPortion())));
    return found.stream()
        .map(record -> view(record, byRecord.getOrDefault(record.getId(), List.of())))
        .toList();
  }

  private DayView view(DailyRecord record, List<LabelAssignmentView> assignments) {
    return new DayView(
        record.getRecordDate(),
        record.getNote(),
        assignments.stream().sorted(Comparator.comparing(LabelAssignmentView::name)).toList());
  }

  private static boolean isValidPortion(BigDecimal portion) {
    return VALID_PORTIONS.stream().anyMatch(valid -> valid.compareTo(portion) == 0);
  }
  ...
  private LabelAssignmentView assignmentView(Label label, BigDecimal portion) {
    return new LabelAssignmentView(label.getId(), label.getName(), label.getColor(), portion);
  }
  ...
  private Label label(UUID userId, UUID id) {
    return labels
        ...
        .filter(value -> scopes.existsByIdLabelIdAndIdScope(id, LabelScopeType.CALENDAR))
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Calendar label not found"));
  }
  ...
  private static void validateColor(String color) {
    if (color != null && !VALID_COLORS.contains(color.toUpperCase(Locale.ROOT)))
      badRequest("Color must be selected from the calendar palette");
  }
